--[[
    Filename:    TeamHolyShopBuyDialog.lua
    Author:      <qiaohuan@playcrab.com>
    Datetime:    2018-01-30 16:54:57
    Description: File description
--]]

local TeamHolyShopBuyDialog = class("TeamHolyShopBuyDialog",BasePopView)
function TeamHolyShopBuyDialog:ctor()
...
-- 第一次被加到父节点时候调用
function TeamHolyShopBuyDialog:onAdd()

end
-- 初始化UI后会调用, 有需要请覆盖
function TeamHolyShopBuyDialog:onInit()
    ...
    self._typeMap = {
		...
		runeCoin = {tipDes = "TIP_GLOBAL_LACK_RUNECOIN", img = ""},
    }
    ...
    self:listenReflash("UserModel", function( )
        if self._costType=="gem" or self._costType=="runeCoin" then
			local canCostNum = self._modelMgr:getModel("UserModel"):getData()[self._data.costType] or 0
...
-- 接收自定义消息
function TeamHolyShopBuyDialog:reflashUI(funcParam)
	local data = funcParam.data
	self._indexId = funcParam.indexId
	...
	local isTool = true
    if not toolD or data.itemType=="rune" then
		isTool = false
    ...
    if self._isAvatarFrame then
        local param = {itemId = itemId,itemData = toolD,eventStyle = 0}
        icon = IconUtils:createHeadFrameIconById(param)
    elseif isTool then
        icon = IconUtils:createItemIconById({itemId = itemId, num = data.num})
	else
	...
	if isTool then
		...
		self._itemName:setColor(UIUtils.colorTable["ccColorQuality".. toolD.quality or 1])
	end
	...
	if isTool then
		...
		local _, haveNum = self._teamModel:getRunesById(itemId)
		haveCount = haveNum
	end
	print("haveNum = " .. haveCount)
    --[[ 获得资源数量
    if haveNum == 0 then
    ...
    self._itemNum:setString((haveCount or "0") .. numTail)
    ...
    --
    self._des2:setPositionX(self._itemNum:getPositionX()+self._itemNum:getContentSize().width+string.len(haveCount or 0)*1.2+1) 
    -- 传入购买回调
    self._closeCallback = funcParam.closeCallback
...
function TeamHolyShopBuyDialog:buyItem()
	local param = {id = self._indexId, type="rune"}
    ...
    self._serverMgr:sendMsg("ShopServer", "buyShopItem", {id = self.awardId,itemId = self.itemId,["type"] = self._shopBuyType}, true, {}, function(result)
        ...
        if self._awardData and self._awardData.award and (self._awardData.award[1] == "avatar" or self._awardData.award[1] == "avatarFrame") then
            ...
            tempdata[2] = self._awardData.itemId
