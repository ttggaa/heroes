--[[
    Filename:    TeamHolySeeDialog.lua
    Author:      <qiaohuan@playcrab.com>
    Datetime:    2018-01-02 15:30:52
...
-- 预览
local TeamHolySeeDialog = class("TeamHolySeeDialog", BasePopView)

function TeamHolySeeDialog:ctor()
    TeamHolySeeDialog.super.ctor(self)
end


function TeamHolySeeDialog:onInit()
    ...
    -- self._effect1 = self:getUI("bg.scrollViewBg.effect1")
    -- self._effect2 = self:getUI("bg.scrollViewBg.effect2")
    -- self._effect3 = self:getUI("bg.scrollViewBg.effect3")
    -- self._eName1 = self:getUI("bg.scrollViewBg.effect1.name")
    -- self._eName2 = self:getUI("bg.scrollViewBg.effect2.name")
    -- self._eName3 = self:getUI("bg.scrollViewBg.effect3.name")
    -- self._richTextBg1 = self:getUI("bg.scrollViewBg.effect1.richTextBg")
    -- self._richTextBg2 = self:getUI("bg.scrollViewBg.effect2.richTextBg")
    -- self._richTextBg3 = self:getUI("bg.scrollViewBg.effect3.richTextBg")
...
function TeamHolySeeDialog:getHolySuitData()
    local suitData = {}
...
function TeamHolySeeDialog:reflashUI(data)
    self._tableView:reloadData()

end

function TeamHolySeeDialog:updateLeftPanel()
...
function TeamHolySeeDialog:updateRightTab(holyData)
...
function TeamHolySeeDialog:updateRightPanel()
    local maxHeight = 0
...
--     local holyId = self._holyId or 101
--     local suitId = tonumber(holyId .. "0" .. self._tabSelect)
--     print("updateRightPanel===============", suitId)
--     local suitTab = tab:Rune(suitId)

--     local maxHeight = 0
--     local richtextBg = self._richTextBg1
--     local richText = richtextBg.richText
--     if richText then
--         richText:removeFromParent()
--     end
...
--     richtextBg:addChild(richText)
--     richtextBg.richText = richText
--     maxHeight = maxHeight + richText:getInnerSize().height


--     local richtextBg = self._richTextBg2
--     local richText = richtextBg.richText
--     if richText then
--         richText:removeFromParent()
--     end
...
--     local richtextBg = self._richTextBg3
--     local richText = richtextBg.richText
--     if richText then
--         richText:removeFromParent()
--     end
...
function TeamHolySeeDialog:refreshTabData(sender, key)
   
    self._tabSelect = sender.__qualityNum or 1
    self:updateRightPanel()
end

function TeamHolySeeDialog:updateCell(inView, indexId, holyData)
...
function TeamHolySeeDialog:setButtonEvent(holyId)
...
用tableview实现
--]]
function TeamHolySeeDialog:addTableView()
...
-- 返回cell的数量
function TeamHolySeeDialog:numberOfCellsInTableView(table)
   return self:getTableNum()
end

function TeamHolySeeDialog:getTableNum()
   return table.nums(self._tableData)
end

-- cell的尺寸大小
function TeamHolySeeDialog:cellSizeForTable(table,idx) 
    local width = 95 
    local height = 86
    return height, width
end
...
-- 创建在某个位置的cell
function TeamHolySeeDialog:tableCellAtIndex(table, idx)
...
function TeamHolySeeDialog:tabButtonClick(sender, key)
   if sender == nil then 
        print("==sender is nil============")
        return 
    end
...
-- 选项卡状态切换
function TeamHolySeeDialog:tabButtonState(sender, isSelected)
...
function TeamHolySeeDialog.dtor()
    qualityName = nil
end
return TeamHolySeeDialog
