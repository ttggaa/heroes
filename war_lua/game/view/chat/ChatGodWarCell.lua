--[[
    Filename:    ChatGodWarCell.lua
    Author:      <wangyan@playcrab.com>
    Datetime:    2016-05-20 12:26
    Description: 联盟单个cell
--]]

local ChatGodWarCell = class("ChatGodWarCell", cc.TableViewCell)

function ChatGodWarCell:ctor()
...
function ChatGodWarCell:onInit()
end

function ChatGodWarCell:reflashUI(data, richText, width, height, isMyself, oldUI)
    ...
    if isMyself == true then 
        self._nameLab:setPosition(width - self._nameLab:getContentSize().width - 255 + 56, height - 9)
    else
        self._nameLab:setPosition(132 - 56, height - 9)
    end
    ...
    if userData.vipLvl and userData.vipLvl ~= 0 then
        ...
        if isMyself == true then
            self._vipImg:setPosition(width - self._nameLab:getContentSize().width - 289 + 56, height - 10)
        else
            self._vipImg:setPosition(138 + self._nameLab:getContentSize().width - 56, height - 7)
        end
    else
    ...
    if isMyself == true then 
        if self._vipImg and self._vipImg:isVisible() then
            self._secLab:setPosition(self._vipImg:getPositionX() - self._secLab:getContentSize().width - 10, height - 9)
        else
            self._secLab:setPosition(self._nameLab:getPositionX() - self._secLab:getContentSize().width - 10, height - 9)
        end
        
    else
        if self._vipImg and self._vipImg:isVisible() then
            self._secLab:setPosition(self._vipImg:getPositionX() + 36, height - 9)
        else
            self._secLab:setPosition(self._nameLab:getPositionX() + self._nameLab:getContentSize().width + 10, height - 9)
        end
    end
...
--创建语音
function ChatGodWarCell:createVoiceItem(data, richText, width, height, isMyself, oldUI)
...
--创建普通信息
function ChatGodWarCell:createOtherItem(data, richText, width, height, isMyself, oldUI)
    local parentNode
    if isMyself == true then
        self._textBg:setVisible(false)
        self._myTextBg:setVisible(true)
