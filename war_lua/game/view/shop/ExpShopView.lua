--[[
    Filename:    ExpShopView.lua
    Author:      <lishunan@playcrab.com>
    Datetime:    2017-08-11 19:17:29
    Description: File description
--]]

-- -- 商店
local ExpShopView = class("ExpShopView",BasePopView)
function ExpShopView:ctor(param)
...
function ExpShopView:onInit()
    self:registerClickEventByName("bg.mainBg.closeBtn", function ()
        ...
        self:close()
        UIUtils:reloadLuaFile("shop.ExpShopView")
    end)
...
-- 接收自定义消息
function ExpShopView:reflashUI(data)
    self:reflashShopInfo()
end

--获取id
function ExpShopView:getItemID(reward)
    if reward[1] == "tool" then
        return reward[2]
    else
        return IconUtils.iconIdMap[reward[1]]
    end
end

-- 按类型返回商店数据
function ExpShopView:getGoodsData( tp )
...
function ExpShopView:updateShopItem()
    local goodsData = self:getGoodsData("exp")
    if not goodsData then 
        return 
    end
...
function ExpShopView:createItem(index, data,x,y)
    -- dump(data)
    local item
    ...
    -- 修正信息
    -- local nowTime = self._modelMgr:getModel("UserModel"):getCurServerTime()
    -- local shopInfo = self._shopModel:getShopGoods("exp")[tostring(data.id)]
    -- if shopInfo then
...
-- 创建空格子
function ExpShopView:createGrid(x,y,pos,data )
    local item = ccui.ImageView:create()
    item:loadTexture("globalImageUI_shop_noneBg.png",1)
...
-- 灰态
function ExpShopView:setNodeColor( node,color )
...
function ExpShopView:sendGetShopInfoMsg( shopName )
    self._serverMgr:sendMsg("ShopServer", "getShopInfo", {type = shopName}, true, {}, function(result)
        if result.shop["exp"] and  
            result.shop["exp"].lastUpTime and 
            result.shop["exp"].lastUpTime > self._nextRefreshTime then 
            self._nextRefreshTime = self._shopModel:getShopRefreshTime("exp")
        end
    end)
end

function ExpShopView:sendReFreshShopMsg( shopName )
    local curTime = self._modelMgr:getModel("UserModel"):getCurServerTime()
    local lastUpTime = self._shopModel:getShopByType("exp").lastUpTime
    ...
    local times = self._shopModel:getShopByType("exp").reflashTimes or 0
    times = times+1
    ...
    local cost = tab:ReflashCost(times)["shopExp"]
    if cost > haveNum then
        ...
        DialogUtils.showBuyDialog({costNum = cost,costType = costType,goods = "刷新一次",callback1 = function( )  
            ...
            self._serverMgr:sendMsg("ShopServer", "reflashShop", {type = "exp"}, true, {}, function(result) 
            end)
        end})
    end
