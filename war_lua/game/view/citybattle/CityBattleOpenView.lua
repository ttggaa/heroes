...
--[[
    Filename:    CityBattleOpenView.lua
...
local CityBattleOpenView = class("CityBattleOpenView",BasePopView)
function CityBattleOpenView:ctor(data)
...
-- 初始化UI后会调用, 有需要请覆盖
function CityBattleOpenView:onInit()
end

-- 第一次进入调用, 有需要请覆盖
function CityBattleOpenView:onShow()

end

function CityBattleOpenView:getMaskOpacity()
    return 0
end

-- 接收自定义消息
function CityBattleOpenView:reflashUI(data)
	self._bg = self:getUI("bg")
	self._target = data.target or self._bg
    local mcStar = mcMgr:createViewMC( "lingtuzhengduokaiqi_kaiqianim", false, false, function (_, sender)
...
function CityBattleOpenView:showFlyAnim( )
    ...
    icon:runAction(cc.Sequence:create(
        ...
        cc.CallFunc:create(function ()
            ...
            self._callback()
            self:close()
			UIUtils:reloadLuaFile("citybattle.CityBattleOpenView")
        end), 
        cc.RemoveSelf:create(true)))
end

return CityBattleOpenView