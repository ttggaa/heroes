--[[
    Filename:    LeagueLocalRankView.lua
    Author:      <wangguojun@playcrab.com>
    Datetime:    2017-06-16 17:33:50
    Description: File description
--]]

local LeagueLocalRankView = class("LeagueLocalRankView",BasePopView)
function LeagueLocalRankView:ctor()
    self.super.ctor(self)
    self._leagueModel = self._modelMgr:getModel("LeagueModel")
end

-- 初始化UI后会调用, 有需要请覆盖
function LeagueLocalRankView:onInit()
	self:registerClickEventByName("bg.closeBtn", function ()
		self._leagueModel:clearRankList()
        self:close()
        UIUtils:reloadLuaFile("league.LeagueLocalRankView")
    end)
	local rankInfo = self._leagueModel:getLocalRank()
 ...
 --    end)
    ...
    self._tableData = self._leagueModel:getLocalRank()
    self._offsetY = nil
	self:addTableView()
...
function LeagueLocalRankView:reflashUserInfo()
	-- self._selfAward:setVisible(true)
	local item  = self._rankItem
	local leagueD = self._leagueModel:getLeague()
	local rank = self._leagueModel:getData().localRank or  self._leagueModel:getData().historyRank
	...
	local hightPoint = self._leagueModel:getLeague().historyPoint or self._leagueModel:getLeague().currentPoint or ""
	levelLab:setString(hightPoint or 1000)
	local UIscoreLab = item:getChildByFullName("scoreLab")
	local zone = self._leagueModel:getCurZone()
	local score = lang(tab:LeagueRank(zone or 1).name)
 ...
 --        destxt:addChild(rtx, 99)
	...
	self:createRoleHead(headInfo,headNode,0.6)
end

function LeagueLocalRankView:getInRangeData( rank )
...
function LeagueLocalRankView:reflashAwardImg( awards )
...
function LeagueLocalRankView:getMyDefScore( )
	-- 战斗力
...
function LeagueLocalRankView:reflashNo1( data )
    ...
    if not tolua.isnull(self._leftBoard._roleAnim) then
	...
	self:registerClickEventByName("bg",function( )
		self._serverMgr:sendMsg("LeagueServer", "GetInfo", {id = data.rid}, true, {}, function(result) 
			-- dump(data,"data==?")
			if next(result) then
				local data1 = clone(result)
				data1.rank = 1
...
function LeagueLocalRankView:addTableView( )
...
function LeagueLocalRankView:createLoadingMc()
...
function LeagueLocalRankView:scrollViewDidScroll(view)
    ...
    if self._inScrolling then
    	local tab = self._leagueModel:getNextLocalRankTab()
		local isOut32 = tab > 2 or (self._tableData and #self._tableData>32)
		if not isOut32 then
        ...
        if self._canRequest and condY == offsetY and not isOut32 then
            self._canRequest = false
            -- 请求数据
            self:createLoadingMc()
            self._loadingMc:setVisible(false)
...
function LeagueLocalRankView:scrollViewDidZoom(view)
    -- print("scrollViewDidZoom")
end

function LeagueLocalRankView:tableCellTouched(table,cell)
    print("cell touched at index: " .. cell:getIdx())
end

function LeagueLocalRankView:cellSizeForTable(table,idx) 
    return self._tableCellH+5,self._tableCellW
end

function LeagueLocalRankView:tableCellAtIndex(table, idx)
...
function LeagueLocalRankView:numberOfCellsInTableView(table)
...
function LeagueLocalRankView:setSlider( )
...
-- 接收自定义消息
function LeagueLocalRankView:reflashUI(data)
...
function LeagueLocalRankView:createItem( data,index )
...
function LeagueLocalRankView:createRoleHead(data,headNode,scaleNum)
	local avatarName = data.avatar
...
function LeagueLocalRankView:getBmpFromNum( num,node,offsetx )
...
--刷新之后tableView 的定位
function LeagueLocalRankView:searchForPosition()	
	self._tableData = self._leagueModel:getLocalRank()

	local tab = self._leagueModel:getCurrLocalRankTab()
...
function LeagueLocalRankView:sendGetRankMsg()
	local tab = self._leagueModel:getNextLocalRankTab()
	if tab > 2 or (self._tableData and #self._tableData>32) then return end
	self._serverMgr:sendMsg("LeagueServer", "getSecRank", {page=tab}, true, {}, function(result) 
