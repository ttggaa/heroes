--[[
    Filename:    GodWarResultDialog.lua
    Author:      <qiaohuan@playcrab.com>
    Datetime:    2017-05-04 22:14:27
    Description: File description
--]]

-- 最终结果展示
local GodWarResultDialog = class("GodWarResultDialog", BasePopView)

function GodWarResultDialog:ctor(param)
    GodWarResultDialog.super.ctor(self)
    self._callback = param.callback
end

function GodWarResultDialog:onInit()
...
function GodWarResultDialog:reflashUI(data)
    ...
    self._godWarModel:setGodWarShowType(balance)
    ...
    self:animBegin(function( )
        ...
        --     print("bg1Height=======", bg1Height, maxHeight)
...
function GodWarResultDialog:nextAnimFuncLayer2(powId)
...
function GodWarResultDialog:nextAnimFunc(powId)
    if powId == 2 then
    ...
    ScheduleMgr:delayCall(50, self, function()
        if not self._titleImg then return end
        -- local scale1 = cc.ScaleTo:create(0, 3)
        -- local scale2 = cc.ScaleTo:create(0.2, 1)
        -- local spawn = cc.Spawn:create(cc.FadeIn:create(0.2), scale2)
        -- local seq = cc.Sequence:create(scale1, spawn)
        -- self._titleImg:runAction(seq)
        -- local mcShua = mcMgr:createViewMC("shuxingshuguang_itemeffectcollection", true, false, function (_, sender)
        --     sender:removeFromParent()
        -- end,RGBA8888)
...
function GodWarResultDialog:setPopAnim1(inView, powId)
...
function GodWarResultDialog:setPopAnim2(inView, powId)
...
function GodWarResultDialog:animBegin(callback)
    self._bg = self:getUI("bg")
...
function GodWarResultDialog:getMaskOpacity()
    return 230
end

function GodWarResultDialog:updateLayer8()
    local powId = 8
    local warData = self._godWarModel:getWarDataById(powId)
    local layer8 = self:getUI("bg.layer8")
    ...
    self._maxLevel:setString(lang("GODWARPAI_1"))
    self._maxLevel:setVisible(false)

    layer8:setVisible(true)
    for i=1,4 do
        local indexId = i*2-1
        local battleData = warData[tostring(i)]
...
function GodWarResultDialog:setPopAnim(inView, powId)
    for i=1,powId do
...
function GodWarResultDialog:updateLayer4()
    local powId = 4
    local warData = self._godWarModel:getWarDataById(powId)
    ...
    self._maxLevel:setString(lang("GODWARPAI_2"))
    self._maxLevel:setVisible(false)
    if not warData then
        return
    end
...
function GodWarResultDialog:updateLayer2()
    local powId = 2
    local warData = self._godWarModel:getWarDataById(powId)
    ...
    self._maxLevel:setString(lang("GODWARPAI_3"))
    self._maxLevel:setVisible(false)
...
function GodWarResultDialog:updateLayer3()
    local powId = 3
    local warData = self._godWarModel:getWarDataById(powId)
    local layer = self:getUI("bg.layer2")
    for i=1,1 do
...
function GodWarResultDialog:updateLayer1()
