--[[
    Filename:    IntanceMcPlotView.lua
    Author:      <weiwei02@playcrab.com>
    Datetime:    2017-02-02 11:22:04
    Description: File description
--]]


local IntanceMcPlotView = class("IntanceMcPlotView", BaseView)
require "game.view.intance.IntanceConst"
function IntanceMcPlotView:ctor(inData)
    IntanceMcPlotView.super.ctor(self)
    self.noSound = true
    self._plotId = inData.plotId
    self._callback = inData.callback
    self._sysPlot = tab:MainPlot(self._plotId)
    self._skipReward = inData.skipReward
end

function IntanceMcPlotView:onInit()
    ...
    self:registerScriptHandler(function(eventType)
        if eventType == "exit" then 
            UIUtils:reloadLuaFile("intance.IntanceMcPlotView")
...
--! @return
--]]
function IntanceMcPlotView:initPlot(inPlotInfo)
    local plotBg = cc.Sprite:create("asset/bg/plot/" .. inPlotInfo.bg .. ".jpg")
    ...
    mc1:setPosition(0, plotBg:getContentSize().height * plotBg:getScale() + 30)
...
--! @return
--]]
function IntanceMcPlotView:initPlot1(inPlotInfo)
...
--! @return
--]]
function IntanceMcPlotView:listenerFrame(inMc, inFrameEvent)
...
--! @return
--]]
function IntanceMcPlotView:initPlotPic(inPic, callback)
    local bgLayer = ccui.Layout:create()
    bgLayer:setBackGroundColorOpacity(0)
    bgLayer:setBackGroundColorType(1)
    bgLayer:setBackGroundColor(cc.c3b(0, 0, 0))
...
--! @return
--]]
function IntanceMcPlotView:showPoint(x, y, callback, notAmin, inScale)
...
function IntanceMcPlotView:getAsyncRes()
...
function IntanceMcPlotView:onDestroy()
    -- audioMgr:playMusic("mainmenu", true)
    IntanceMcPlotView.super.onDestroy(self)
end


function IntanceMcPlotView:destroy(removeRes)
    IntanceMcPlotView.super.destroy(self, removeRes)
end

function IntanceMcPlotView:isAsyncRes()
    return false
end

function IntanceMcPlotView:setNoticeBar()
    self._viewMgr:hideNotice(true)
end

return IntanceMcPlotView