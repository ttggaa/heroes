--[[
    Filename:    IntanceBranchView.lua
    Author:      <weiwei02@playcrab.com>
    Datetime:    2015-12-15 10:54:23
    Description: File description
--]]

local IntanceBranchView = class("IntanceBranchView", BasePopView)


function IntanceBranchView:ctor()
    IntanceBranchView.super.ctor(self)
    self._usingIcon = {}
    self._freeingIcon = {}
    self:registerScriptHandler(function(eventType)
        if eventType == "exit" then 
            UIUtils:reloadLuaFile("intance.IntanceBranchView")
...
function IntanceBranchView:reflashUI(inData)
  ...
  for i=1,8 do
    ...
    if bg ~= nil then
      ...
      if ADOPT_IPHONEX then
        bg:getChildByName("Image_9"):setScaleX(1.3)
      end
    end
  end
  ...
  if sysBranchStage.type == IntanceConst.STAGE_BRANCH_TYPE.REWARD_ITEM_TEAM then 
    branchType = IntanceConst.STAGE_BRANCH_TYPE.REWARD_TEAM
  end
  print("branchType=====", branchType, branchId)
  ...
  if branchType == IntanceConst.STAGE_BRANCH_TYPE.WAR or 
        branchType == IntanceConst.STAGE_BRANCH_TYPE.REWARD_TEAM or 
        branchType == IntanceConst.STAGE_BRANCH_TYPE.COST_TEAM or
        branchType == IntanceConst.STAGE_BRANCH_TYPE.COST_ITEM_CHIP then
        ...
        callbackFunction = function()
          if sysBranchStage.type == IntanceConst.STAGE_BRANCH_TYPE.WAR then
            ...
            return
          elseif branchType == IntanceConst.STAGE_BRANCH_TYPE.COST_TEAM or 
            branchType == IntanceConst.STAGE_BRANCH_TYPE.COST_ITEM_CHIP then
    ...
    elseif branchType == IntanceConst.STAGE_BRANCH_TYPE.REWARD_ITEM or 
      branchType == IntanceConst.STAGE_BRANCH_TYPE.TIP or 
      branchType == IntanceConst.STAGE_BRANCH_TYPE.STAR then 
        local itemIcon = bg:getChildByName("itemIcon")
        itemIcon:loadTexture("asset/uiother/intance/" .. sysBranchStage.tipPic .. '.png')
        callbackFunction = function()
          self._callback(branchId, 2)
        end
        itemIcon:setScale(sysBranchStage.zoom/100)
    elseif branchType == IntanceConst.STAGE_BRANCH_TYPE.REWARD_HERO then 
  ...
  -- 战报回放特殊处理
  if branchType == IntanceConst.STAGE_BRANCH_TYPE.WAR then
  ...
  -- 奖励内容特殊处理
  if branchType == IntanceConst.STAGE_BRANCH_TYPE.COST_TEAM or 
            branchType == IntanceConst.STAGE_BRANCH_TYPE.COST_ITEM_CHIP then
    ...
    if branchType == IntanceConst.STAGE_BRANCH_TYPE.STAR then
...
function IntanceBranchView:onInitSpecial(sysBranchStage)
  local bg = self:getUI("bgSpecial")
  bg:setVisible(true)
  local branchType = sysBranchStage.type
  if branchType ~= IntanceConst.STAGE_BRANCH_TYPE.WAR then
...
function IntanceBranchView:showReport(branchId, inSubType, inCallback)
    --回放
