...
local DialogZombieRank = class("DialogZombieRank",BasePopView)
function DialogZombieRank:ctor(params) 
...
local bossId = 5
-- 初始化UI后会调用, 有需要请覆盖
function DialogZombieRank:onInit()
	...
	self:registerClickEventByName("bg.closeBtn", function ()
        ...
        self:close()
        UIUtils:reloadLuaFile("pve.DialogZombieRank")
    end)
...
function DialogZombieRank:reflashUserInfo()
	...
	scoreNum:setString("累计伤害: " .. totalScore)
	...
	if rank == 0 or not rank then
		...
		local award = cryptWeeklyReward[1].award[1]
	    ...
	    desTxt:setString("排名进入" .. cryptWeeklyReward[1].pos[2] .. "名可获得：")
    else
		for k,v in pairs(cryptWeeklyReward) do
			local pos = v.pos
...
function DialogZombieRank:reflashNo1( data )
	...
	self:registerClickEventByName("bg.leftBoard",function( )
			self._serverMgr:sendMsg("BossServer", "getPVERankDetailInfo", {bossId = 5,roleId = data.rid}, true, {}, function(result) 
...
function DialogZombieRank:addTableView( )
...
function DialogZombieRank:scrollViewDidScroll(view)
	self._inScrolling = view:isDragging()
    -- self:setSlider()
end

function DialogZombieRank:scrollViewDidZoom(view)
    -- print("scrollViewDidZoom")
end

function DialogZombieRank:tableCellTouched(table,cell)
    print("cell touched at index: " .. cell:getIdx())
end

function DialogZombieRank:cellSizeForTable(table,idx) 
    return self._tableCellH+5,self._tableCellW
end

function DialogZombieRank:tableCellAtIndex(table, idx)
...
function DialogZombieRank:numberOfCellsInTableView(table)
...
--[[
function DialogZombieRank:setSlider( )
...
-- 接收自定义消息
function DialogZombieRank:reflashUI(data)
...
function DialogZombieRank:createItem( data,index )
	...
	UIscoreLab:setString(self:formatScore(score))
	local rankLab = item:getChildByName("rankLab")
	if not rankLab then
		rankLab = cc.Label:createWithTTF("0", UIUtils.ttfName, 28) --cc.LabelBMFont:create("4", UIUtils.bmfName_rank)
	    rankLab:setAnchorPoint(cc.p(0.5,0.5))
...
function DialogZombieRank:createRoleHead(data,headNode,scaleNum)
	local avatarName = data.avatar
...
function DialogZombieRank:formatScore(score)
	local num = score
	local result = 0
	if score > 10000 then
		result = string.format("%.1f",num / 10000) .. "万"
	else
		result = num
	end 
	return result
...
function DialogZombieRank:getBmpFromNum( num,node,offsetx )
	...
	while pos <= endPos do
		...
		if numC then 
			local numSp = ccui.ImageView:create("pveRankUI_" .. numC .. ".png",1)
...
function DialogZombieRank:sendGetRankMsg()
	self._serverMgr:sendMsg("ArenaServer", "getRank", {}, true, {}, function(result) 
		self:reflashUI()
    end)
end
function DialogZombieRank.dtor()
	-- body
	bossId = nil
	rankImgs = nil
	rankTextColor = nil
end
return DialogZombieRank