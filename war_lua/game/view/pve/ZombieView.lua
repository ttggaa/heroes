--[[
    Filename:    ZombieView.lua
    Author:      <liushuai@playcrab.com>
    Datetime:    2015-11-03 16:19:59
    Description: File description
--]]

local FormationIconView = require("game.view.formation.FormationIconView")

local ZombieView = class("ZombieView", BaseView)

local PVEID = 902
local PVETYPE = 5

ZombieView.kGiftItemTag = 1000

function ZombieView:ctor()
    ZombieView.super.ctor(self)
...
function ZombieView:getAsyncRes()
    return 
        {
...
function ZombieView:getBgName()
    return "bg_zombie.jpg"
end

function ZombieView:disableTextEffect(element)
    if element == nil then
...
function ZombieView:onInit()
    ...
    label_enemy_recommand:setString(lang("TIPS_PVE_CRYPT_01"))
    label_enemy_recommand:enableOutline(zombieOutlineColor, 1)
    ...
    titleLabel1:setColor(cc.c3b(250,224,188))
    titleLabel1:enableOutline(zombieOutlineColor, 2)
...
--            icon = IconUtils:createTeamIconById({teamData = {id = teamId, star = teamTableData.star}, sysTeamData = teamTableData, quality = backQuality[1], quaAddition = backQuality[2], tipType = 9, eventStyle = 2})
    ...
    self._labelTimes:enableOutline(zombieOutlineColor, 1)
    ...
    self._labelTimesValue:enableOutline(zombieOutlineColor, 1)
...
--    image_battle:addChild(amin2)
    ...
    self:registerClickEvent(self:getUI("bg.rankNode.rankBg"), function()
        ...
        self._serverMgr:sendMsg("BossServer", "getPVEScoreRank", {bossId = PVETYPE}, true, {}, function(success)    
            self._viewMgr:showDialog("pve.DialogZombieRank", {callback = function()
    ...
    myRankTxt:enableOutline(zombieOutlineColor, 1)
    ...
    self:registerClickEvent(rankNode:getChildByFullName("btn_rule"), function()
        self._viewMgr:showDialog("pve.PveRuleView", {viewType = 2}, true, true)
    end)

    self:registerClickEvent(rankNode:getChildByFullName("btn_reward"), function()
        self._viewMgr:showDialog("pve.ZombieRewardView", {viewType = 2}, true, true)
    end)
...
--判断推荐 wangyan
function ZombieView:checkRecomment()
...
function ZombieView:onSweepBtnClicked()
    ...
    local sweepFunc = function()        
        self._serverMgr:sendMsg("BossServer", "sweepBoss", {id = PVEID}, true, {}, function(success, result)
            ...
            local params = {gifts = result.reward,pveDes=lang("PVE_SAODANG_902")}
    ...
    local isAwakingTask = self._modelMgr:getModel("AwakingModel"):getAwakingTaskZombieCondition()
    -- isAwakingTask = true
    ...
    if self:checkRecomment() == true then
        ...
        self._serverMgr:sendMsg("BossServer", "getSweepReward", {id = PVEID}, true, {}, function(result,success)
            ...
            local curDesc = string.gsub(lang("TIPS_PVE_CRYPT_03"), "{$texp}", ItemUtils.formatItemCount(num))
            DialogUtils.showShowSelect({desc = curDesc, addValue = "zombie",callback1=function( )
                sweepFunc()
            end})
        end)
    end
...
function ZombieView:formatButton(btn)
...
-- 更新左下角排行信息      
function ZombieView:updateRankListPanel(rankList)
    local bossData = self._bossModel:getDataByPveId(PVETYPE) or {}
...
-- 根据排名 获得奖励数量
function ZombieView:getAwardNumByRank(rank)
...
function ZombieView:addRewardNotice()
    ...
    local awardD = tab["cryptDailyReward"] 
...
function ZombieView:onHide( )
    self._viewMgr:disableScreenWidthBar()
end
function ZombieView:onComplete()
    self._viewMgr:enableScreenWidthBar()
end
function ZombieView:onDestroy( )
    self._viewMgr:disableScreenWidthBar()
    ZombieView.super.onDestroy(self)
end

function ZombieView:onModelReflash()
    if self._bossModel:isNeedRequest() then
        self:doRequestData()
    else
        self:updateUI()
...
function ZombieView:doRequestData()
    self._serverMgr:sendMsg("BossServer", "getBossInfo", {}, true, {}, function(success)
        self:updateUI()
    end)
end

function ZombieView:doBattle()
    local bossData = self._bossModel:getDataByPveId(PVETYPE)
    local times = 0
    if bossData then
        times = bossData.times or 0
    ...
    -- 战斗内需要展示最高伤害记录
    local maxDamage = self._bossModel:getMaxDamage(2)
    ...
    local battle = function(battleData)
        self._serverMgr:sendMsg("BossServer", "beforeAttackBoss", {id = PVEID, serverInfoEx = BattleUtils.getBeforeSIE()}, true, {}, function(errorCode, result)
            ...
            local addScend = self._modelMgr:getModel("PrivilegesModel"):getAbilityEffect(PrivilegeUtils.privileg_ID.PRIVILEGENAME_3) or 0
            local addHp = self._modelMgr:getModel("PrivilegesModel"):getAbilityEffect(PrivilegeUtils.privileg_ID.PRIVILEGENAME_4) or 0 
            local addHulan = 2 -- 填写1或者2
            local playerInfo = BattleUtils.jsonData2lua_battleData(result["atk"])
            playerInfo.maxDamage = maxDamage
            BattleUtils.enterBattleView_Zombie(PVEID, playerInfo, addHp, addHulan, addScend,
    ...
    self._viewMgr:showView("formation.NewFormationView", {
        formationType = self._formationModel.kFormationTypeZombie,
...
function ZombieView:onIconPressOn(icon)
    print("onIconPressOn")
end

function ZombieView:onIconPressOff()
    print("onIconPressOff")
end

function ZombieView.dtor( ... )
    -- body
    FormationIconView = nil
    PVEID = nil
    PVETYPE = nil
    zombieOutlineColor = nil
end
