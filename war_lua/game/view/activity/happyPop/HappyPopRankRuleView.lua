--[[
    Filename:    HappyPopRankRuleView.lua
    Author:      <wangyan@playcrab.com>
    Datetime:    2017-11-20 15:40:10
    Description: 消消乐 规则界面
--]]

local HappyPopRankRuleView = class("HappyPopRankRuleView",BasePopView)

function HappyPopRankRuleView:ctor(param)
    ...
    self._sysData = tab.magicTrainingRank
end

-- 初始化UI后会调用, 有需要请覆盖
function HappyPopRankRuleView:onInit()
    ...
    for i=1, 3 do
        local rwdIconT = self:getUI("bg.rankCell.rwd" .. i)
        local rwdNumT = self:getUI("bg.rankCell.rwdNum" .. i)
        rwdIconT:setVisible(false)
        rwdNumT:setVisible(false)
    end
    ...
    -- 增加富文本
	local rtxStr = lang("MAGICTRAINING_RANK_RULE")
    local acData = self._hPopModel:getAcData()
    if acData and next(acData) then
        local endM = TimeUtils.getDateString(acData.end_time - 86400,"%d")
        rtxStr = string.gsub(rtxStr, "{$day}", endM)
    end
...
function HappyPopRankRuleView:listenModelHandle(inData)
...
function HappyPopRankRuleView:getInRangeData( rank )
...
function HappyPopRankRuleView:reflashTitleInfo()
    ...
    if honorD then
        self:createRewards(tip2, honorD["rewards"])
    end
end

function HappyPopRankRuleView:generateRanks()
    ...
    for i,rankD in ipairs(self._sysData) do
        ...
        --rank
        local rankStr = ""
        if i == #self._sysData then
            local pos = rankD.rank
            rankStr = "第" .. pos[1] .. "名之后"
        else
            local pos = rankD.rank
            rankStr = "第" .. getRange(pos[1], pos[2]) .. "名"
        end
        ...
        --rwd
        self:createRewards(item, rankD["rewards"])

        self._scrollBg:addChild(item)
    end

    -- 奖励顶部描述
    local rwdTip = lang("MAGICTRAINING_RANK_TIPS1")
    local acData = self._hPopModel:getAcData()
    if acData and next(acData) then
        local endM = TimeUtils.getDateString(acData.end_time - 86400,"%d")
        rwdTip = string.gsub(rwdTip, "{$day}", endM)
    end
    local topDes = RichTextFactory:create(rwdTip, 418, height)
    topDes:formatText()
...
function HappyPopRankRuleView:createRewards(inObj, inRwas)
