...
-- end
local mcName = {
    "huanyingshengong_treasurehuanyingshengong",
    "guiwangdoupeng_treasureguiwangdoupeng",
    "acERecharge_soulStone.png",
}
...
function ACEveryDayRechargeLayer:onInit()
    ...
    self._pageView = self:getUI("bg.showTrsu.pageView")
...
-- 提示文字
    ...
    minGem:loadTexture("acERecharge_tipImg4.png", 1)
    minGem:setPositionY(minGem:getPositionY()-10)
...
function ACEveryDayRechargeLayer:updateUI()
    ...
    showTrsuBg:setPositionX(showTrsuBg:getPositionX()-10)
    showTrsuBg:setPositionY(showTrsuBg:getPositionY()-15)
    local bg = self:getUI("bg")
    ...
    for i=1,3 do
        ...
        local showTrsu
        if i == 3 then
            showTrsu = ccui.ImageView:create()
            showTrsu:loadTexture(mcName[i],1)
            -- showTrsu:setScale(2)
            local acArray = {}
            table.insert(acArray,cc.MoveBy:create(0.7,cc.p(0,5)))
            table.insert(acArray,cc.MoveBy:create(0.7,cc.p(0,-5)))
            showTrsu:runAction(cc.RepeatForever:create(cc.Sequence:create(acArray)))
        else
            showTrsu = mcMgr:createViewMC(mcName[i], true, false)
            showTrsu:setScale(0.7)
        end
        
        showTrsu:setPosition(cc.p(layout:getContentSize().width/2, layout:getContentSize().height/2))
        layout:addChild(showTrsu)
        self._pageView:insertPage(layout,i-1)
    end

    local curIdx = self._pageView:getCurPageIndex()
    print("curIdx ",curIdx)
    local leftPanel = ccui.Layout:create()
    leftPanel:setAnchorPoint(cc.p(0.5,0.5))
    leftPanel:setTouchEnabled(true)
    leftPanel:setContentSize(cc.size(50,50))
    self._leftArrow = mcMgr:createViewMC("zuojiantou_teamnatureanim", true, false)
    self._leftArrow:setPosition(25,25)
    leftPanel:addChild(self._leftArrow)
    leftPanel:setPosition(35,277)
    bg:addChild(leftPanel, 99)

    local rightPanel = ccui.Layout:create()
    rightPanel:setAnchorPoint(cc.p(0.5,0.5))
    rightPanel:setTouchEnabled(true)
    rightPanel:setContentSize(cc.size(50,50))
    self._rightArrow = mcMgr:createViewMC("youjiantou_teamnatureanim", true, false)
    self._rightArrow:setPosition(15,25)
    rightPanel:addChild(self._rightArrow)
    rightPanel:setPosition(279,277)
    bg:addChild(rightPanel, 99)
...
function ACEveryDayRechargeLayer:moveToItem(add)
    local curIdx = self._pageView:getCurPageIndex()
    print("curIdx ",curIdx)
    local nextIdx = curIdx + add
    self._pageView:scrollToPage(nextIdx)
    -- if nextIdx >= 3 then
    --     local layout = self._pageView:getPage(0)
    --     self._pageView:insertPage(layout,2)
    -- elseif nextIdx <= 0 then
    --     local layout = self._pageView:getPage(3)
    --     self._pageView:insertPage(layout, 0)
    -- end
    for i=0,2 do
        local node = self._pageView:getPage(i)
        local curIdx = self._pageView:getCurPageIndex()
        if i ~= curIdx then
