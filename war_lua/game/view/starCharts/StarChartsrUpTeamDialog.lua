...
--
local StarChartsrUpTeamDialog = class("StarChartsrUpTeamDialog",BasePopView)
function StarChartsrUpTeamDialog:ctor(param)
...
-- 加载资源
-- function StarChartsrUpTeamDialog:getAsyncRes()
--     return 
--     {
--         {"asset/ui/battle.plist", "asset/ui/battle.png"},        
--     }
-- end
function StarChartsrUpTeamDialog:getMaskOpacity()
    return 230
end

-- 初始化UI后会调用, 有需要请覆盖
function StarChartsrUpTeamDialog:onInit()
...
function StarChartsrUpTeamDialog:animBegin(callback)
    -- 播放获得音效
...
-- 接收自定义消息
function StarChartsrUpTeamDialog:reflashUI(data)
...
function StarChartsrUpTeamDialog:createItem( data,x,y,index,nextFunc )
            ...
            ScheduleMgr:delayCall(120*index, self, function( )
                ...
                self:registerClickEventByName("closePanel", function()
                    if not hadClose then
                        ...
                        UIUtils:reloadLuaFile("starCharts.StarChartsrUpTeamDialog")
                    end
                end)
...
-- 获得兵团原始数据
function StarChartsrUpTeamDialog:getTeamAttrValue(teamData,attrId)
    local attrs = {}
    ...
    for i=BattleUtils.ATTR_Atk, BattleUtils.ATTR_COUNT do
        backData[i] = backData[i] + heroAttr[i]
    end
    -- end copy
    --星图属性
    local starTeamAttr = self._modelMgr:getModel("UserModel"):getStarTeamAttr()
    
    if attrId == 3 or attrId == 6 then
        backData[3] = backData[3] + starTeamAttr[3]
        backData[6] = backData[6] + starTeamAttr[6]
        attrs[6] = BattleUtils.getTeamHpAttr(backData, true)        -- 生命
        attrs[3] = BattleUtils.getTeamAttackAttr(backData, true)    -- 攻击
    else
        attrs[attrId] = backData[attrId] + starTeamAttr[attrId]
    end
...
-- 数值变化动画
function StarChartsrUpTeamDialog:runValueChangeAnim( label,endDes )
...
return StarChartsrUpTeamDialog