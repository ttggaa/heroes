--[[
 	@FileName 	SiegeRankView.lua
	@Authors 	zhangtao
	@Date    	2017-09-18 14:42:40
	@Email    	<zhangtao@playcrad.com>
	@Description   描述
--]]
local SiegeRankView = class("SiegeRankView",BasePopView)
function SiegeRankView:ctor()
...
local rankTable = {
	[28] = tab:Setting("G_RANK_ELEMENTAL_SHOW_1").value,
	[29] = tab:Setting("G_RANK_ELEMENTAL_SHOW_2").value,
}
function SiegeRankView:ctor(data)
    self.super.ctor(self)
...
-- 初始化UI后会调用, 有需要请覆盖
function SiegeRankView:onInit()
    self:registerClickEventByName("bg.layer.closeBtn", function()
    	self._rankModel:clearRankList()
        self:close()
        UIUtils:reloadLuaFile("siege.SiegeRankView")
    end )

    self:registerClickEventByName("bg.layer.rankItem.ruleBtn",function()
...
function SiegeRankView:updateUI()
	...
	-- end
	self:reflashTitleName(1)	

end

function SiegeRankView:updateTableData(rankList,index)
	-- print("*************************",index)
	local data = {}
...
function SiegeRankView:reflashUI(data) 
	self:reloadRankUI()
end

function SiegeRankView:reloadRankUI()
    self._tableData = self._rankModel:getRankList(self._rankType)[self._stageId] or {}
...
function SiegeRankView:reflashNoRankUI()
	if not self._tableData or #self._tableData <= 0 then
		self._noRankBg:setVisible(true)
		self._rankItem:setVisible(false)
		self._titleBg:setVisible(false)
...
function SiegeRankView:reflashTitleName(index)
...
function SiegeRankView:reflashUserInfo()
	...
	local type = tab.siegeMainStage[self._stageId]["type"]
	local hurtValue = rankDataScore
    local hurtValueNode = item:getChildByFullName("hurtValue")
    hurtValueNode:setString(hurtValue)
	local ranknum = math.min(rank or 10000,10000)
	-- local ranknum = 1000
	local rewardD = self._siegeModel:getInRangeData(ranknum,self._stageId)
	local itemBg = item:getChildByFullName("awardBg")
	local noRank = item:getChildByFullName("noRank")
	if rewardD then
		self:createTopRankNode(itemBg,rewardD["award"])
...
function SiegeRankView:reflashNo1( data )
...
function SiegeRankView:addTableView( )
...
function SiegeRankView:createLoadingMc()
...
function SiegeRankView:scrollViewDidScroll(view)
...
--是否要刷新排行榜
function SiegeRankView:sendMessageAgain()
	self._allRankData = self._rankModel:getRankList(self._rankType)[self._stageId] or {}
...
--刷新之后tableView 的定位
function SiegeRankView:searchForPosition(statCount,addCount,endCount)
...
--获取排行榜数据
function SiegeRankView:sendGetRankMsg(tp,start,callback)
...
function SiegeRankView:scrollViewDidZoom(view)
    -- print("scrollViewDidZoom")
end

function SiegeRankView:tableCellTouched(table,cell)
    print("cell touched at index: " .. cell:getIdx())
end

function SiegeRankView:cellSizeForTable(table,idx) 
    return 80,546
end

function SiegeRankView:tableCellAtIndex(table, idx)
    ...
    if nil == cell then
	    ...
	    self:createItem(item,cellData,idx+1)
...
function SiegeRankView:numberOfCellsInTableView(table)
	return #self._tableData
end

function SiegeRankView:createItem(item,data,index )
	...
	-- levelLab:setPositionX(levelLab:getPositionX())
...
--	UIscoreLab:setColor(cc.c4b(70,40,0,255))
	...
	rankLab:setString(rank)
	if headNode.icon then
		headNode.icon:removeFromParent(true)
	end
	...
	-- item:setSwallowTouches(false)
 	...
 	if item.tequanIcon then
		item.tequanIcon:removeFromParent(true)
	end
    --启动特权类型
...
function SiegeRankView:createRoleHead(data,headNode,scaleNum)
...
function SiegeRankView:showCommentView(rankData)
...
-- 根据排行类型获得分数数据
function SiegeRankView:getScoreByRankType(score, kType)
    score = score or 0
    if kType == self._rankModel.kRankTypeSiegeAttack then
        return score
    elseif kType == self._rankModel.kRankTypeSiegeDefend then
        return score
    end
end

-- 将分数分解出通关时间和剩余血量
function SiegeRankView:getTimeAndBloodByScore(score)
...
function SiegeRankView:getBmpFromNum( num,node,offsetx )
...
--排名奖励
function SiegeRankView:createTopRankNode(item,award)
    local posTable = {{x = 0,y=13},{x = 70,y=13},{x=150,y=13}}
    for k , data in pairs(award) do
        ...
        awardLab:setPosition(posTable[k]["x"]+33,posTable[k]["y"]+13)
...
function SiegeRankView.dtor()
    rankImgs = nil
    -- titleTxt = nil
    titleTable = nil
    -- rankTypeTable = nil
end
return SiegeRankView