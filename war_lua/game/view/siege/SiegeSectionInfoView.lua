...
-- Date: 2017-10-17 21:50:02
--
local SiegeSectionInfoView = class("SiegeSectionInfoView", BasePopView)


function SiegeSectionInfoView:ctor()

    SiegeSectionInfoView.super.ctor(self)
    self:registerScriptHandler(function(eventType)
        if eventType == "exit" then 
            UIUtils:reloadLuaFile("siege.SiegeSectionInfoView")
...
function SiegeSectionInfoView:reflashUI(inData)
...
function SiegeSectionInfoView:tabButtonClick(sender)
    if sender == nil then
        return
    end
...
function SiegeSectionInfoView:showTaskNode()
	...
	if sysSectionInfo ~= nil then 
		local str = lang(sysMainSection.des)
	    ...
	    local taskDescBg = self:getUI("bg.bg1.taskDescBg")
	    print("sysMainSection.task==================", sysMainSection.task)
	    ...
	    for k1,v1 in pairs(sysMainSection.task) do
	        ...
	        if targetType == 3 or targetType == 4 then 
	            if self._sModel:isStagePass(targetId) then
	    ...
	    local function showTaskByIndex(inIndex)
	    	...
	    	local taskId = sysMainSection.task[inIndex]
...
function SiegeSectionInfoView:showInfoNode()
	local sysSectionInfo = tab:SiegeSectionInfo(self._curSectionId)
...
function SiegeSectionInfoView:createInfoNode(inSysSectionInfo)
	local height = 0
...
function SiegeSectionInfoView:createTargetNode(inSysSectionInfo)
	...
	for k,v in pairs(inSysSectionInfo.traget) do
		...
		if self._sModel:isStagePass(v) then 
...
function SiegeSectionInfoView:createBranchNode(inSysSectionInfo)
	local height = 0
	...
	local sysMainSection = tab:SiegeMainSection(inSysSectionInfo.id)

	local branchHasEye = {}
	local branchWithStage = self._sModel:getSysBranchWithStageDatas()

    local curStageId = self._sModel:getCurStageId()
    local curStatus = self._sModel:getData().status
    local branchData = self._sModel:getBranchInfo()
	for k,v in pairs(sysMainSection.includeStage) do
		for k1,v1 in pairs(branchData) do
			branchInfo[tonumber(k1)] = true
		end

		if not self._sModel:isStagePass(v) then
	...
	for k,v in pairs(inSysSectionInfo.branchId) do
		local sysBranchStage = tab:SiegeBranchStage(v)
		...
		local temp1 = cc.Sprite:createWithSpriteFrameName("siege_branch4.png")
		...
		if branchInfo[v] ~= nil then 
			local getIt = cc.Sprite:createWithSpriteFrameName("siege_ImageUI_finish.png")
		...
		registerClickEvent(buildingIcon, function()
			...
			if branchHasEye[branchWithStage[v]] == nil then 
				...
				if self._callback ~= nil then 
					self._callback(branchWithStage[v], v)
					self:close()
				end
			end
		end)
...
function SiegeSectionInfoView:createRoleNode(inSysSectionInfo)
	local height = 20
...
function SiegeSectionInfoView:createProgressNode(inSysSectionInfo)
	...
	local minNum = self._sModel:getSectionBranchRate(inSysSectionInfo.id)
    ...
    local progress = ccui.LoadingBar:create("siege_blueLine.png", 1, 100)
	...
	if minNum == maxNum then
		if self._sModel:hasGetMainAward(maxNum) then 
...
--! @return 
--]]
function SiegeSectionInfoView:getBranchAcReward(inSysSectionInfo, inShowType)
	local needNum = inSysSectionInfo.finishReward[1][1]
	local haveNum = self._sModel:getSectionBranchRate(inSysSectionInfo.id)
	local sysMainSection = tab:SiegeMainSection(inSysSectionInfo.id)
    local desc = ""
    local canGet = false
    ...
    if inShowType == nil then
        if viewType == nil then 
            local param = {num = needNum, id = inSysSectionInfo.id}
            self._serverMgr:sendMsg("SiegeServer", "getBranchAcReward", param, true, {}, function (result)
                showGiftGet()
