--[[
    Filename:    GlobalGiftGetDialog.lua
    Author:      <wangguojun@playcrab.com>
    Datetime:    2015-07-30 14:39:47
    Description: File description
    Code refactoring: 2017.4.7 by guojun 
    -- 说明：这个文件涉及需求太多，尽量找原作者修改添加需求
    ------- 该文件对应bg1 展示获得物品(无按钮) bg0 展示可获得物品(有按钮，有小精灵立汇)
    ------- 传入 viewType 是否为空来判断显示bg1(viewType==nil) 或bg0(viewType==1)
    ------- 现有传入参数整理
    -- viewType     显示 bg0 还是bg1
    -- btnTitle     bg0 上按钮名字
    -- gifts        显示的物品数组
    -- des 或 desc  bg0顶部中间的描述  如：XX条件可领取宝箱
    -- title        顶部中间 描述 同des desc
    -- bottomDes    底部描述  如vip加成？
    -- vipPlus      vip签到双倍或几倍加成显示条，在底部
    -- pveDes       pve扫荡提示
--]]
...
function GlobalGiftGetDialog:ctor(data)
    self.super.ctor(self)
    self.callback = data.callback or nil
    self.viewType = data.viewType   -- viewType 不为nil 表示只是展示内容有小精灵立汇，
    ...
    self.bgName = "bg.bg1"
    self._transferStr = ""
    if data.viewType then -- bg.bg0 和 bg.bg1 bg0为有确定按钮，bg1没有
        self.bgName = "bg.bg0"
    end
...
function GlobalGiftGetDialog:onDestroy()
...
function GlobalGiftGetDialog:getMaskOpacity()
    return 230
end
...
function GlobalGiftGetDialog:onInit()
    ...
    if self.viewType then
        ...
        title:setColor(cc.c4b(235,180,90,255))
        -- title:enableOutline(cc.c4b(0, 0, 0, 255),1.5)
        self._title = title
        local hadClose
        self._okBtn:setTitleText(self._btnTitle or "确定") 

        self:registerClickEventByName("bg.bg0.okBtn", function()
            if not hadClose then
                ...
                UIUtils:reloadLuaFile("global.GlobalGiftGetDialog")
            end
        end)
        --翅膀
        local swing1 = self:getUI("bg.bg0.Image_30")
        
        -- swing1:setPosition(247, -6)
        local swing2 = self:getUI("bg.bg0.Image_31")
        swing2:setAnchorPoint(cc.p(0, 0))
        swing2:setPosition(-13, 57)
...
function GlobalGiftGetDialog:animBegin(callback)
...
-- 接收自定义消息
function GlobalGiftGetDialog:reflashUI(data)
    -- dump(data,"...data...in GlobalGiftGetDialog:reflashUI")
    ...
    if self.viewType then
        offsetX = 6--(self.bgWidth-(col-1)*itemWidth)*0.5
    ...
    showItems = function( idx )
        ...
        -- for i=1,#gifts do
            x = x + itemWidth
            if not self.viewType and idx ~= 1 and (idx-1) % colMax == 0 then 
                x =  offsetX
                y = y - itemHeight
            end
            if not self.viewType and idx > 10 and idx%5 == 1 then -- 多一行就 滚屏
...
-- pve扫荡提示
function GlobalGiftGetDialog:addDespveDes(isAdd,pveDes)
...
function GlobalGiftGetDialog:createItem( data,x,y,index,nextFunc )
    local itemData
    ...
    ScheduleMgr:delayCall(120, self, function( )--index*
        ...
        if index == #self._gifts then
            item:runAction(cc.Sequence:create(cc.Spawn:create(cc.FadeIn:create(0.1),cc.ScaleTo:create(0.1,itemNormalScale*0.6)),cc.ScaleTo:create(0.1,itemNormalScale),cc.CallFunc:create(function( )
                ...
                -- if #self._heros then
                --     local function showHero( index )
                --         if self._heros[index] then
                --             local heroView = self._viewMgr:createLayer("hero.HeroUnlockView", {heroId = tonumber(self._heros[index][2] or self._heros[index].typeId), callBack = function() 
                --                 showHero(index+1)
                --             end})
...
function GlobalGiftGetDialog:closeFunc( )
    if self._hideView then
        self._hideView:setVisible(true)
    end
...
-- 设置 scaleAnim
function GlobalGiftGetDialog:processItemsAfterAction( )
...
function GlobalGiftGetDialog:showTreasureNotOpen( )
    -- 宝物开启返回
...
-- 显示远征VIP、活动加成
function GlobalGiftGetDialog:initAbilityEffect()
    if not self.addition or not self.addition.add then
        self:getUI("bg.bg0.addPanel"):setVisible(false)
        return
    end

    local vipImg = self:getUI("bg.bg0.addPanel.vipImg")
    vipImg:loadTexture(self.addition.add.img, 1)
    local acImg = self:getUI("bg.bg0.addPanel.activityImg")
    acImg:loadTexture(self.addition.add.img, 1)
    local vipAddNum = self:getUI("bg.bg0.addPanel.vipImg.addNum")
    local acAddNum = self:getUI("bg.bg0.addPanel.activityImg.addNum")
    vipImg:setVisible(false)
    acImg:setVisible(false)

    local acAddValue = self.addition.add.acAdd
    local vipAddValue = self.addition.add.vipAdd
    ...
    if acAddValue > 0 then
        ...
        acAddNum:setString("+" .. acAddValue .. "%")

        if vipAddValue <= 0 then
            acImg:setPositionX(105)
        end
    end
end
...
function GlobalGiftGetDialog:addVipPlusDes( maxHeight,vipPlus )
...
function GlobalGiftGetDialog:createPlusItem( vipImgName,resImgName,num,vipDes )
    local bgNode = ccui.Widget:create()
    local alignNodes = {}
