--[[
 	@FileName 	HeroSkillMoraleTipView.lua
	@Authors 	yuxiaojing
	@Date    	2018-06-12 17:51:27
	@Email    	<yuxiaojing@playcrab.com>
	@Description   描述
--]]
-- 英雄属性 回魔 
local HeroSkillMoraleTipView = class("HeroSkillMoraleTipView", BaseLayer)
function HeroSkillMoraleTipView:ctor(params)
    HeroSkillMoraleTipView.super.ctor(self)
end

function HeroSkillMoraleTipView:onInit()
	...
	self._initPos["attrCell8"] = self._initPos["attrCell7"]+34
end

function HeroSkillMoraleTipView:setAttrs( totalAttrs )
	...
	self:getUI("bg.attrCell7.des"):setString(totalAttrs.subDes or "")
	...
	for k = 1,cellNum do
		...
		-- attrNum = attrNum*10
		if k ~= 1 and k ~= 7 and attrNum and attrNum > 0 then
	...
	if totalAttrs.averageMCD then
		...
		self._bg:setContentSize(cc.size(290,415+bgAddHeight))
		for k,v in pairs(self._children) do
		...
		if totalAttrs.slotMCD then
			...
			self._bg:setContentSize(cc.size(290,435+bgAddHeight))
			for k,v in pairs(self._children) do
		...
		self._bg:setContentSize(cc.size(290,395+bgAddHeight))
		for k,v in pairs(self._children) do
