--[[
    Filename:    GuildMapAQRankViewll.lua
...
local titleTable = {
	[20] = {[1]="排名",[2]="角色名",[3]="斯芬克斯认可度"},   --个人
	[21] = {[1]="排名",[2]="联盟",[3]="斯芬克斯认可度"},	--联盟
}
local payerShow = clone(tab:Setting("G_RANK_SPHINX_SHOW_1").value)
local guildShow = clone(tab:Setting("G_RANK_SPHINX_SHOW_2").value)

local showTable = {
	[20] = {payerShow[1],payerShow[2]},    	--个人
	[21] = {guildShow[1],guildShow[2]},		--联盟
}
...
local GuildMapAQRankView = class("GuildMapAQRankView", BasePopView)

function GuildMapAQRankView:ctor(param)
...
function GuildMapAQRankView:onInit()
	self:initOriginView()
    ...
    self:registerClickEvent(ruleBtn, function ()
        self._viewMgr:showDialog("guild.map.GuildMapAQRuleView", {curType = self._rankType})
    end)
    ...
    self._guildTipPanel = self:getUI("bg.bgPanel.guildTipPanel")
    self._guildTipPanel:setVisible(false)
    self._guildTipPanel:setSwallowTouches(true)

    self._applyBtn = self:getUI("bg.bgPanel.guildTipPanel.applyBtn")
	...
	-- 递进刷新控制
	self.beginIdx = { [20] = showTable[20][1], [21] = showTable[21][1] }
	self.addStep = { [20] = showTable[20][1], [21] = showTable[21][1] }
	self.endIdx = { [20] = showTable[20][2], [21] = showTable[21][2] }
	...
	for i=1, 2 do
		...
		table.insert(self._tabs, ids[i], tab)
	...
	self._tabs[self._rankType or 20]._appearSelect = true
	self:touchTab(self._rankType or 20)
end

function GuildMapAQRankView:initOriginView()
	...
	self:getUI("bg.bgPanel.rankItem2.nameLab"):setString("")
	self:getUI("bg.bgPanel.rankItem2.scoreLab"):setString("")
	self:getUI("bg.bgPanel.rankItem2.levelLab"):setString("")
end

function GuildMapAQRankView:touchTab( idx )
...
function GuildMapAQRankView:setBtnState()
	local idx = self._rankType
...
function GuildMapAQRankView:updateTableData(rankList)
...
function GuildMapAQRankView:reflashTitleName()
...
function GuildMapAQRankView:reflashUserInfo()
	local item  = self._rankItem
	local sysData 
	if self._rankType == 20 then
		sysData = tab.sphinxPersonRank
	else
		sysData = tab.sphinxGuildRank
	end
	...
	local function createRewards(inRank)
		...
		local rwds = sysData[inRank]["reward"]
		for i = 1, 3 do
...
function GuildMapAQRankView:createRoleHead(data,headNode,scaleNum)
...
function GuildMapAQRankView:reflashNo1( data )
...
function GuildMapAQRankView:addTableView( )
...
function GuildMapAQRankView:createLoadingMc()
...
function GuildMapAQRankView:scrollViewDidScroll(view)
...
function GuildMapAQRankView:tableCellTouched(table,cell)
    
end

function GuildMapAQRankView:cellSizeForTable(table,idx) 
    return self._tableCellH+5,self._tableCellW
end

function GuildMapAQRankView:tableCellAtIndex(table, idx)
...
function GuildMapAQRankView:numberOfCellsInTableView(table)
	return #self._tableData
end

-- 接收自定义消息
function GuildMapAQRankView:reflashUI(data)
	local offsetX = nil
	local offsetY = nil
...
function GuildMapAQRankView:reflashNoRankUI()
...
function GuildMapAQRankView:createItem( data,index )
...
function GuildMapAQRankView:updateItem20()
...
function GuildMapAQRankView:updateItem21()
	local item = self._currItem
	local data = self._itemData
	local name = data.name	or ""
	local nameLab = item:getChildByFullName("nameLab")
...
function GuildMapAQRankView:itemClicked(data)
	if not data then 
		return 
	end
...
function GuildMapAQRankView:goView20()
...
function GuildMapAQRankView:goView21()
	local param = clone(self._clickItemData)
	param.score = param.zhandouli
	 self._viewMgr:showDialog("guild.dialog.GuildDetailDialog", {allianceD = param})
end

--是否要刷新排行榜
function GuildMapAQRankView:sendMessageAgain()
	self._allRankData = self._rankModel:getRankList(self._rankType)
...
--刷新之后tableView 的定位
function GuildMapAQRankView:searchForPosition(statCount,addCount,endCount)
	self._offsetX = 0
...
--获取排行榜数据
function GuildMapAQRankView:sendGetRankMsg(tp,start,callback)
...
function GuildMapAQRankView.dtor()
    titleTable = nil
	rankImgs = nil
	payerShow = nil
	guildShow = nil
