--[[
    ...
    Datetime:    2016-04-18 21:40:20
    Description: File description
--]]


-- 联盟管理界面
local GuildManageView = class("GuildManageView", BaseView, require("game.view.guild.GuildBaseView"))

function GuildManageView:ctor()
    GuildManageView.super.ctor(self)
    self._membersData = {}
    self.initAnimType = 3
...
function GuildManageView:onInit()
    ...
    -- self._clickCount = 0
    -- local lab1 = self:getUI("bg.bg1.lab1")
    -- lab1:setTouchEnabled(true)
    -- self:registerClickEvent(lab1,function ()
    --     print("aaaaaa")
    --     if self._clickCount < 20 then
    --         self._clickCount = self._clickCount + 1
    --     else
    --         if self._curentSeeData then
    --             local tips1 = self._curentSeeData.online or "online nil"
    --             local tips2 = self._curentSeeData.leaveTime or "leaveTime nil"
    --             self._viewMgr:showTip("online:"..tips1.."  leaveTime:" .. tips2)
    --         end
    --     end
    -- end)
end
...
-- -- 重写 GuildBaseView:reflashQuitAlliance()
-- function GuildManageView:reflashQuitAlliance()
--     local userData = self._userModel:getData()
--     local hintTip = self:getUI("bg.huizhang.agreeBtn.hintTip")
--     if userData.guildApply ~= 0 then
--         hintTip:setVisible(true)
...
-- 更新公告
function GuildManageView:updateGonggao()
...
-- 设置按钮
function GuildManageView:setBtnFont()
    -- 改名
    ...
    local changeName = self._btnLayer:getChildByFullName("changeNameBtn") -- self:getUI("bg.changeName")
    if changeName then
    ...
    -- 审批管理
    local agreeBtn = self._btnLayer:getChildByFullName("agreeBtn") -- self:getUI("bg.exitBtn")
    if agreeBtn then
        self:registerClickEvent(agreeBtn, function()
            if self:checkCondition(2) then
                return
            end
    ...
    self:registerClickEvent(libao, function()
        sdkMgr:loadUrl({url = GameStatic.wxGiftUrl})
        print(GameStatic.wxGiftUrl)
    end)
...
function GuildManageView:checkCondition(checkType)
...
-- -- 设置按钮
-- function GuildManageView:setBtnPos(_type)
--     if true then
--         return
--     end
--     -- 审批管理
...
-- 更新绑定状态
function GuildManageView:updateBindBtn()
    local bindBtn = self._btnLayer:getChildByFullName(self._pingtai)
    ...
    elseif status == 0 then 
        ...
        print("=====================")
        print(bindGroup.hadJoin)
        print("=====================")
        print(type(bindGroup.hadJoin))
        print("=====================")
        ...
        elseif bindGroup.hadJoin == nil or bindGroup.hadJoin == 0 then --修改 如果联盟已绑定群,所有未在群里的人都显示加入按钮
            tempBtnNum = 2
            bindBtn:setTitleText("  加入群")
            print("bindPlatformGroup============加入1===")
            registerClickEvent(bindBtn, function()
                self._requestBindState = 1
                local param = {}
                param.union_id = tostring(userData.guildId)
...
-- 返回游戏获取绑定信息
function GuildManageView:applicationWillEnterForeground()
        ...
        if roleGuild.pos == 1 then 
            if bindGroup and bindGroup.hadBind and tonumber(bindGroup.hadBind) == 1 then
                self._serverMgr:sendMsg("GuildServer", "getGameGuildInfo", {}, true, {}, function (result)
                    if result then
                        dump(result,"GuildManageView:applicationWillEnterForeground=",10)
                        self:updateBindBtn()
                    end
                end)
            else
...
function GuildManageView:isGetGem()
    local isGetGem = nil
    local userData = self._userModel:getData()
    local roleGuild = userData.roleGuild
    dump(roleGuild, "isGetGem=======" , 10)
...
function GuildManageView:showGemReward(param)
    print("GuildManageView:showGemReward")
...
function GuildManageView:getBindGuildInfo()
    ...
    self._serverMgr:sendMsg("GuildServer", "getBindGuildInfo", {}, true, {}, function (result)
        ...
        -- if not _isGetGem and status then
        --     self:showGemReward()
        -- end
...
-- 解绑
function GuildManageView:unBindGuild()
    self:showDialog("global.GlobalMessageDialog", {desc = "是否确认解除绑定?", button = "确认", callback = function ()
...
function GuildManageView:bindGuild()
    print("GuildManageView:bindGuild")
    local _isGetGem = self:isGetGem()
    if _isGetGem then
        print("GuildManageView:bindGuild 1")
    else
        print("GuildManageView:bindGuild 2")
    end
    self._serverMgr:sendMsg("GuildServer", "bindGuild", {}, true, {}, function (result)
        if result == nil then 
            return
        end
        dump(result,"bindGuild===",10)
        self._guildModel:getAllianceDetail().bindGroup = result
        self:updateBindBtn()
        ...
        if not _isGetGem and status_ == true then
...
function GuildManageView:reflashUI(data)
    print("刷新数据")
    dump(data, "data ----------====")
    local guildModel = self._modelMgr:getModel("GuildModel")
...
function GuildManageView:refreshNotReload()
...
function GuildManageView:updateCell(inView, data)
    if data == nil then
        return
    end
    -- dump(data,"data ====================")
    ...
    --修改为显示玩家登录时间 2017.08.07
    local loginTime = inView:getChildByFullName("loginTime")
    print(data.name,TimeUtils.getDateString(data.lt))
    if loginTime then
...
--    if inView.qqVipIcon == nil then
--        local qqVipIcon = ccui.ImageView:create(qqVipImg, 1)
--        qqVipIcon:setScale(24 / qqVipIcon:getContentSize().width)
--        qqVipIcon:setPosition(name:getPositionX() + name:getContentSize().width + 16, name:getPositionY())
--	    inView:addChild(qqVipIcon)
--        inView.qqVipIcon = qqVipIcon
--    else
--        inView.qqVipIcon:loadTexture(qqVipImg, 1)
--        inView.qqVipIcon:setPosition(name:getPositionX() + name:getContentSize().width + 16, name:getPositionY())
--    end
end

-- 退出联盟
function GuildManageView:quitGuild()
    print("载入=============数据====")
...
-- function GuildManageView:quitGuildFinish(result)
--     if result == nil then 
--         return 
--     end
-- end

function GuildManageView:createAvatar()
...
-- 快速加入
function GuildManageView:quickJoinGuild()
...
function GuildManageView:onBeforeAdd(callback, errorCallback)
...
function GuildManageView:getGuildInfo1()
    self._serverMgr:sendMsg("GuildServer", "getGameGuildInfo", {}, true, {}, function (result)
...
function GuildManageView:getGuildInfo()
    self._serverMgr:sendMsg("GuildServer", "getGameGuildInfo", {}, true, {}, function (result)
        dump(result)
...
function GuildManageView:getGuildInfoFinish(result)
    -- dump(result,"result ===================")
    if result == nil then
...
function GuildManageView:getAsyncRes()
    return {
...
-- 渲染时会调用, 改变元件坐标在这里
function GuildManageView:onAdd()

end
-- 切换动画完毕时候调用, 界面动画在这里, 发请求最好也在这里
function GuildManageView:onAnimEnd()
    if (not sdkMgr:isOpenBindGroup()) and (not sdkMgr:isOpenJoinGroup()) then
        return
    end
...
-- 查看日志
function GuildManageView:getGuildEvent()
...
用tableview实现
--]]
function GuildManageView:addTableView()
...
-- cell的尺寸大小
function GuildManageView:cellSizeForTable(table,idx) 
    local width = 582 
    local height = 106
    return height, width
end
...
-- 创建在某个位置的cell
function GuildManageView:tableCellAtIndex(table, idx)
    ...
    if nil == cell then
        ...
        playerCell:setPosition(cc.p(2,0))
