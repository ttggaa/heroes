--[[
    Filename:    TestRoleSpriteFrameAnim.lua
    Author:      <huachangmiao@playcrab.com>
    Datetime:    2017-05-16 14:58:36
    Description: File description
--]]

local TestRoleSpriteFrameAnim = class("TestRoleSpriteFrameAnim", BaseView)
local AnimAP = require "base.anim.AnimAP"
package.loaded["base.anim.AnimAP"] = nil

function TestRoleSpriteFrameAnim:ctor()
    TestRoleSpriteFrameAnim.super.ctor(self)

    self._roleList = {}
    local data
    for k, v in pairs(AnimAP) do
    	if k ~= "mcList" then
	    	data = clone(v)
	    	data.name = k
...
function TestRoleSpriteFrameAnim:onDestroy()
    local dispatcher = cc.Director:getInstance():getEventDispatcher()
    dispatcher:removeEventListenersForTarget(self._roleNode, true)
	TestRoleSpriteFrameAnim.super.onDestroy(self)
end

local motionString = {"stop","run","atk","die","atk2","atk3","atk4","born"}
function TestRoleSpriteFrameAnim:onInit()
    ...
    self:registerClickEvent(btn1, function ()
    	...
    	if self._motionIndex < 1 then
    		self._motionIndex = #motionString
    	end
        self:onUpdate()
    ...
    self:registerClickEvent(btn1, function ()
    	self._motionIndex = self._motionIndex + 1
    	if self._motionIndex > #motionString then
    		self._motionIndex = 1
    	end
        self:onUpdate()
    end)
...
function TestRoleSpriteFrameAnim:onMouseScroll(event)
...
function TestRoleSpriteFrameAnim:onUpdate()
	self._roleNode:removeAllChildren()
	-- print(self._motionIndex)
	self._motionlabel:setString(motionString[self._motionIndex])
	self._motionlabel1:setString(motionString[(self._motionIndex - 1) < 1 and #motionString or self._motionIndex - 1])
	self._motionlabel2:setString(motionString[(self._motionIndex + 1) > #motionString and 1 or self._motionIndex + 1])
	...
	SpriteFrameAnim.new(self._roleNode, data.name, function (_sp)
        _sp:changeMotion(self._motionIndex)
        _sp:play()
        -- 定位点
        ...
        rect:setOpacity(128)
        rect:setScale(data["H"] / 80)
        self._roleNode:addChild(rect)
        if data[3] then
            local sp4 = cc.Sprite:create("asset/other/circle.png")
            sp4:setColor(cc.c3b(0, 255, 255))
            sp4:setOpacity(150)
            sp4:setScale(0.2)
            sp4:setPosition(data[3][1], data[3][2])
        ...
        _sp:setVisible(not self._showImage)
        rect:setVisible(not self._showImage)
    end, self._ex)

    local image 
    if self._ex then
        image = cc.Sprite:create("asset/role/"..data.name.."_e.png")
    else
        image = cc.Sprite:create("asset/role/"..data.name..".png")
    end
...
function TestRoleSpriteFrameAnim:updateAnimAP()
...
return TestRoleSpriteFrameAnim
