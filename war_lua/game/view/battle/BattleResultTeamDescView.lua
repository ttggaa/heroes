...
function BattleResultTeamDescView:updateTeamInformation()
    ...
    local nameStr = nil
    local rolePic = nil
    if isJx and not tab.npc[self._teamData.ID] then
    ...
    self._team._layerClass:setBackGroundImage(IconUtils.iconPath .. TeamUtils.getNpcTableValueByTeam(teamTempData, "classlabel") .. ".png", 1)
    ...
    self._team._labelZiZhiDes:setString(12 + teamTempData.zizhi)
    ...
    for i = 1, 4 do
        ...
        if teamData.sl then
            skillLevel = teamData.sl[i]
        elseif skillData then
            skillLevel = skillData[i]
        end
        local skillType = teamSkillData[i][1]
        local skillId = teamSkillData[i][2]
