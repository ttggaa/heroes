--[[
 	@FileName 	BattleResultElementWin.lua
	@Authors 	zhangtao
	@Date    	2017-09-14 10:58:26
	@Email    	<zhangtao@playcrad.com>
	@Description   描述
--]]
local BattleResultElementWin = class("BattleResultElementWin", BasePopView)

function BattleResultElementWin:ctor(data)
	-- dump(data,"aaaaaaaaaaaaaa",10)
    BattleResultElementWin.super.ctor(self)
...
function BattleResultElementWin:getBgName()
    return "battleResult_bg.jpg"
end

function BattleResultElementWin:onInit()
    ...
    if self._result.reward or self._result.rewards or self._rewards  then
	    ...
	    local initTeamIconFunc = function(id,i)
	    	...
	    	if teamModel:getTeamAndIndexById(id) or isMercenary then
...
function BattleResultElementWin:initResIconPos(count)
	if count == 5 then
...
function BattleResultElementWin:initTxPlus()
...
function BattleResultElementWin:initExpCoin()
	print("BattleResultElementWin:initExpCoin")
...
function BattleResultElementWin:onQuit()
	...
	if self._callback then
		self._callback()
        UIUtils:reloadLuaFile("battle.BattleResultElementWin")
	end
	-- 对好友切磋特做
...
function BattleResultElementWin:onCount()
	self._viewMgr:showView("battle.BattleCountView",self._battleInfo,true)
end

function BattleResultElementWin:animBegin()	
	audioMgr:stopMusic()
	...
	if teamData then
    	...
    	if isMercenary then
			...
			tdata = teamModel:getTeamAndIndexById(lihuiId)
		end	    	            
...
function BattleResultElementWin:starPanelAction()	
	local starP = self:getUI("bg.starPanel")
	self._delayT = 0
...
function BattleResultElementWin:animNext(mc2)
	if mc2 == nil then
		return
	end
...
function BattleResultElementWin:labelAnimTo(label, src, dest, isTime)
...
function BattleResultElementWin.dtor()
	BattleResultElementWin = nil
	-- delaytick = nil
	-- soundDelaytick = nil
end


return BattleResultElementWin