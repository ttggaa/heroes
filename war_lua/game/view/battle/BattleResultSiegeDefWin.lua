--[[
    @FileName   BattleResultSiegeDefWin.lua
    @Authors    zhangtao
    @Date       2017-10-09 20:40:48
    @Email      <zhangtao@playcrad.com>
    @Description   守城战
--]]
local BattleResultSiegeDefWin = class("BattleResultSiegeDefWin", BasePopView)

function BattleResultSiegeDefWin:ctor(data)
    BattleResultSiegeDefWin.super.ctor(self)
...
function BattleResultSiegeDefWin:getBgName()
    return "battleResult_bg.jpg"
end
function BattleResultSiegeDefWin:onInit()
    ...
    --守城进度
    self._monsterName4 = self:getUI("bg.bg1.des4")
    self._monsterName4:enableOutline(cc.c4b(0,0,0,255),1)
    self._count4 = self:getUI("bg.bg1.count4")
    self._count4:enableOutline(cc.c4b(0,0,0,255),1)
    --女王鼓舞
    ...
    local damageValue = self._result["exInfo"]["damageCount"]
    ...
    --击退波数
    local waves = self._result["exInfo"]["waveCount"]
    local maxWaves = siegeModel._maxMaves
    self._count3:setString(waves .."波")
    if tonumber(waves) > tonumber(maxWaves) and tonumber(maxWaves) ~= 0 then
        self._upLab11:setString("(")
        self._upLab12:setString((waves - maxWaves) .. " )")
        self._upNum1:setVisible(true)
        self._upNum1.isShow = true
    end
    self._upNum1:setPositionX(self._count3:getPositionX() + self._count3:getContentSize().width + 30)

    --守城进度
    local progressValue = 0
    local stageId = self._result["extend"]["stageId"]
    local maxWavesCount = tab.siegeMainStage[stageId]["enemyAmount"]
    if tonumber(waves) >= tonumber(maxWavesCount) then
        progressValue = 100
        self._count4:setString("+"..progressValue .."%")
    else
        progressValue = string.format("%0.3f",((waves*100)/maxWavesCount))
        -- progressValue = 0.001
        print("=====progressValue======"..progressValue)
         print("=====waves======"..waves)
          print("=====maxWavesCount======"..maxWavesCount)
        if tonumber(progressValue) ~= 0 then
            local buffNum = self._result["extend"]["buff"] or 0
            local addBuffHurt = waves*(tonumber(buffNum)/100)
            local addProgress = string.format("%0.4f",(addBuffHurt*100/maxWavesCount))
            if tonumber(addProgress) == 0 then
                addProgress = 0.0001
            end
            self._count4:setString("+"..tonumber(progressValue) .."%")
            self._buff:setString("(鼓舞+"..tonumber(addProgress).."%)")
            self._buff:setVisible(true)
        end
        if tonumber(progressValue) == 0 and waves > 0 then
            progressValue = "<".. 0.001
            self._count4:setString(progressValue .."%")
            self._buff:setVisible(false)
        end        
    ...
    -- if tonumber(progressValue) ~= 0 then
    --     self._buff:setString("(鼓舞+"..buffNum.."%)")
    --     self._buff:setVisible(true)
    -- end


    self._buff:setPositionX(self._count4:getPositionX() + self._count4:getContentSize().width)
    --我的排名
    ...
    if tonumber(oldRank) > tonumber(newRank) then
        self._upLab21:setString("(")
    ...
    --判断最后一波是否被别人击退
    local lastWaves = siegeModel:getData()["waves"] or 0
    if isLast == 0 and lastWaves == 0 then
        self._viewMgr:showTip(lang("TIPS_SIEGE_WARNINGTIPS2"))
    end
end

function BattleResultSiegeDefWin:onQuit()
    if self._callback then
        self._callback()
        -- UIUtils:reloadLuaFile("battle.BattleResultSiegeDefWin")
    end
end

function BattleResultSiegeDefWin:onCount()
    self._viewMgr:showView("battle.BattleCountView",self._battleInfo,true)
end

local delaytick = {360, 380, 380}
function BattleResultSiegeDefWin:animBegin(isWin)
    audioMgr:stopMusic()
    if isWin then
...
function BattleResultSiegeDefWin:animNext(isWin, mc2)
    local animPos = self:getUI("bg.animPos")
    if isWin then
...
function BattleResultSiegeDefWin:labelAnimTo(label, src, dest, isTime)
...
function BattleResultSiegeDefWin.dtor()
    BattleResultSiegeDefWin = nil
    delaytick = nil
    -- diffList = nil
end

return BattleResultSiegeDefWin