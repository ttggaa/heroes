--[[
    Filename:    BattleRule_GuideSiege.lua
    Author:      huachangmiao@playcrab.com
    Datetime:    2016-11-20 16:37:44
...
function BattleLogic:initLogicEx(procBattle)
    ...
    for i = 1, 4 do
        if self._siegeD["pylonid"][i] and self._siegeD["pylonid"][i] ~= 0 then
            self._pylons[i] = self:addPylon(self._siegeD["pylonid"][i], 26 - i, posx1[i], posy1[i])
        end
    end
    ...
    for i = 1, 4 do
        if self._siegeD["arrowid"][i] and self._siegeD["arrowid"][i] ~= 0 then
            self._arrows[i] = self:addArrow(self._siegeD["arrowid"][i], posx1[i] - 1, posy1[i], altitude[i])
        end  
    end
    ...
    if self:hasAssist2() then
        ...
        if not BATTLE_PROC then
            ...
            self._heros[1]:setPos(x - INTO_DIS, 12 * 40)
        end
...
-- 添加可攻击城墙
function BattleLogic:addPylon(npcid, motion, x, y)
...
-- 添加弓箭手
function BattleLogic:addArrow(npcid, x, y, altitude)
    local team = BattleTeam.new(ECamp.RIGHT)   
    local info = {npcid = npcid, level = LEVEL2, summon = false, number = 1}
    BC.initTeamAttr_Npc(team, self._heros[2], info, x, y, 1.5, true)    
    self:_raceCountAdd(2, team.race1, team.race2)
    self.classCount[2][team.classLabel] = self.classCount[2][team.classLabel] + 1
    team.noHUD = true
    self:addTeam(team)     
...
function BattleLogic:_specialHelp(npcid, x, y, action, value, camp)
    if action == 1 then
        ...
        team.attackArea = -1
        team.__help = true
        team.soldier[1]:changeMotion(17)
        if self._helpDelayCallTick == nil then
            self._helpDelayCallTick = {}
        end
        self._helpDelayCallTick[team] = value
        objLayer:hideTeamHUD(team)  
        self:addToUpdateList(team)
    elseif action == 2 then
        objLayer:playEffect_skill1("senlvchuxian2_senlvchuxian", x, y, false, false, nil, 2)
        objLayer:playEffect_skill1("senlvchuxian1_senlvchuxian", x, y, true, true, nil, 2)
        delayCall(value, self, function()
            local team = BattleTeam.new(ECamp.LEFT)   
            local info = {npcid = npcid, level = 1, summon = false}
            BC.initTeamAttr_Npc(team, self._heros[1], info, x, y)    
            self:_raceCountAdd(1, team.race1, team.race2)
    ...
    if self._help1Tick then
        if self._help1Tick <= self.battleTime then
            ...
            for i = 2, #help1 do
                ...
                local action, value = help1[i][4][1], help1[i][4][2]  
                x, y = BC.MAX_SCENE_WIDTH_PIXEL * 0.5 + x, BC.MAX_SCENE_HEIGHT_PIXEL * 0.5 + y
                self:_specialHelp(npcid, x, y, action, value, 1)
            end    
        end
    end
    if self._help2Tick then
        if self._help2Tick <= self.battleTime then
            ...
            for i = 2, #help2 do
                ...
                local action, value = help2[i][4][1], help2[i][4][2]  
                x, y = BC.MAX_SCENE_WIDTH_PIXEL * 0.5 + x, BC.MAX_SCENE_HEIGHT_PIXEL * 0.5 + y
                self:_specialHelp(npcid, x, y, action, value, 2)
            end        
        end
    end

    if self._helpDelayCallTick then
        for team, value in pairs(self._helpDelayCallTick) do
            if value <= self.battleTime then 
                local soldier = team.soldier[1]
                delayCall(0.1, self, function()
                    audioMgr:playSound("shitouren_1")
                end)
                delayCall(1.35, self, function()
                    logic:soldierCastSkill(false, tab.skill[50218], 1, BC.updateCasterPos(soldier), soldier, true)
                    logic:shake(3, 5)
                end)
                delayCall(2.45, self, function()
                    logic:shake(5, 5)
                end)
                delayCall(4.3, self, function()
                    objLayer:showTeamHUD(team)  
                    team.canDestroy = true
                    if team.state == ETeamStateIDLE then
                        team:setState(ETeamStateMOVE)
                    end
                    if team.__attackArea then
                        team.attackArea = team.__attackArea
                    end
                end)
                team.soldier[1]:changeMotion(8)
                self._helpDelayCallTick[team] = nil
                if next(self._helpDelayCallTick) == nil then
                    self._helpDelayCallTick = nil
                end
            end
        end
    end
...
function BattleLogic:teamAttackOverEx(team)
    if team.camp == 2 and self._aliveSiegesCount > 0 then
        team:setState(ETeamStateIDLE)
    else
        team:stopMove(true)
...
function BattleLogic:onTeamDieEx(team)
    ...
    if self._aliveSiegesCount == 0 then
        ...
        for i = 1, #self._teams[1] do
            ...
            -- 非亡灵 士气高涨
            if _team.state ~= ETeamStateDIE then
                if _team.race1 ~= 104 then
                    if not BC.jump then
                        objLayer:playEffect_skill1("shiqigaozhang_shiqieff", _team.x, _team.y, true, true)
                    end
                    list = _team.aliveSoldier
                    for i = 1, #list do
                        buff = initSoldierBuff(BUFF_ID_SHIQIGAOZHANG, 1, list[i].caster, list[i])
                        list[i]:addBuff(buff)
                    end
                    -- 士气高涨触发技能
                    if #list > 0 then
...
function rule.dtor()
    ...
    ETeamStateIDLE = nil -- BC.ETeamState.IDLE
    ETeamStateMOVE = nil -- BC.ETeamState.MOVE
    ETeamStateNONE = nil -- BC.ETeamState.NONE
    ETeamStateDIE = nil
