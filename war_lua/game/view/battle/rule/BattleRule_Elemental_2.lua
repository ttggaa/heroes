--[[
    Filename:    BattleRule_Elemental_2.lua
...
function BattleSoldier:HPChange(attacker, _damage, crit, _damageKind, dieKind, noAnim, param1, bufflabel)
    if self.ID == BOSS1_ID then
        if logic._shieldCurHP > 0 then
            -- 有盾的情况，免疫伤害
            if (bufflabel == nil or BUFF_LABEL_TAB[bufflabel] == nil) then
            ...
            if res < 0 then
                ...
                if logic._shieldCurHP <= 0 then
                    self.___totem._endTick = 0
                    self.___totem = nil
                    self:delBuff(logic._param["buff1"])
                    self:resetAttr()
                    ...
                    if not BATTLE_PROC then
                        if self.__dun1 then
                            objLayer:stopEffect(self.__dun1) 
                            self.__dun1 = nil
                            objLayer:stopEffect(self.__dun2) 
                            self.__dun2 = nil
                            objLayer:playEffect_totemDisappear("die1_suibing", self.x - 90, self.y - 15, true, true, 1.5)
                            objLayer:playEffect_totemDisappear("die2_suibing", self.x + 75, self.y, false, true, 1.5)
                        end
                    end
                end
            end
            ...
            local pro = floor(self.HP / self.maxHP * 100)
            if pro < logic._nextShieldHPPro then
                logic._nextShieldHPPro = logic._nextShieldHPPro - 30
                -- 加盾
                self:changeMotion(5)
                logic._shieldCurHP = logic._shieldMaxHP
                logic.subDestValue2 = 100
                self.team.shieldCur = 100
                self.___totem = logic:addTotemToSoldier(tab.object[logic._param["buff"]], 1, self, self)
                local buff = BC.initSoldierBuff(logic._param["buff1"], 1, self.caster, self)
                self:addBuff(buff)

                if not BATTLE_PROC then
                    self.__dun1 = objLayer:playEffect_totem("stop1_suibing", self.x - 90, self.y - 15, true, true, true, 1.5)
                    self.__dun2 = objLayer:playEffect_totem("stop2_suibing", self.x + 75, self.y, false, true, true, 1.5)
                end
            end
            return res
        end
...
function BattleLogic:initLogicEx()
    ...
    local param = tab.elementSkillPara[2]
    self._param = param
    ...
    self._nextShieldHPPro = 70

    local skillLevel = bossTeam1.D["sl"][1] or 1
    self._shieldMaxHP = floor((param["p4"][1] + (skillLevel - 1) * param["p4"][2]) * boss1.maxHP * 0.01) * 100
    self._shieldCurHP = 0
end
