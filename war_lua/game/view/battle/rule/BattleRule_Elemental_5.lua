--[[
    Filename:    BattleRule_Elemental_5.lua
...
function BattleSoldier:HPChange(attacker, _damage, crit, _damageKind, dieKind, noAnim, param1, bufflabel)
    if self.ID == BOSS1_ID then
        if logic._controled then
            ...
            return 0, true
            -- local __damage = ceil(_damage * 0.01)
            -- return BattleSoldier_HPChange(self, attacker, __damage, crit, _damageKind, dieKind, noAnim)
        end
    else
        return BattleSoldier_HPChange(self, attacker, _damage, crit, _damageKind, dieKind, noAnim)
    end
...
function BattleLogic:initLogicEx()
    ...
    -- 免疫buff
    local immuneBuffTab = {11, 12, 13, 14, 15, 17, 99}
...
function BattleLogic:xuHua()
    if BATTLE_PROC then return end
    objLayer:setPColor(self._boss1.ID, 0, 0, 0, 50, 120)
end

function BattleLogic:puTong()
    if BATTLE_PROC then return end
    objLayer:cancelColor(self._boss1.ID)
end
...
function BattleLogic:updateEx()
    ...
    local controled = self._controled
    local newcontroled = not boss.canAttack and not boss.canMove
    if controled ~= newcontroled then
        if newcontroled then
            self:puTong()
        else
            self:xuHua()
        end
        self._controled = newcontroled
    end
end
