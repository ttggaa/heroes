--[[
    Filename:    BattleRule_Elemental_1.lua
...
function BattleScene:initBattleUIEx()
    ...
    self._subPro2:setColor(cc.c3b(238, 34, 45))

    self._subDestValue1 = 100
    self._subDestValue2 = 100
end
...
-- boss1只收到连击伤害
local LIANJI_SKILL_TAB = 
{
    [50001] = 1, [50069] = 1,  -- 弩手
    [50145] = 1, [50146] = 1,  -- 木精灵
    [50062] = 1, [50063] = 1,  -- 兽人狼骑兵
    [50006] = 1, [50070] = 1,  -- 十字军
    [50184] = 1, [50185] = 1,  -- 幻影射手女
}
function BattleSoldier:HPChange(attacker, _damage, crit, _damageKind, dieKind, noAnim, skillid)
    if self.ID == BOSS1_ID and (skillid == nil or LIANJI_SKILL_TAB[skillid] == nil) then
        if not BATTLE_PROC then if mran(10) == 1 then self:HPanim_immune() end end
        return 0, true
    end
    return BattleSoldier_HPChange(self, attacker, _damage, crit, _damageKind, dieKind, noAnim, skillid)
end
...
function BattleLogic:initLogicEx()
    ...
    local param = tab.elementSkillPara[1]
    
    local bossTeam1 = self._teams[2][1]
    local boss1 = bossTeam1.soldier[1]
    local bossTeam2 = self._teams[2][2]
    local boss2 = bossTeam2.soldier[1]
    ...
    self:addTotemToSoldier(tab.object[param["buff"]], 1, boss1, boss1)
    BOSS1_ID = boss1.ID

    -- 免疫buff
    local immuneBuffTab = {7, 10, 11, 12, 13, 14, 15, 16, 17, 99}
    for i = 1, #immuneBuffTab do
        bossTeam2.immuneBuff[immuneBuffTab[i]] = true
    end

    boss2.immuneForbidMove = true
    boss2.immuneForbidAttack = true
    boss2.immuneForbidSkill = true
    boss2.boss = true

    boss2:resetAttr()
    boss2.maxProDamage = 0.01
    BOSS2_ID = boss2.ID

    self._bossTeam1 = bossTeam1
    self._boss1 = boss1
    self._bossTeam2 = bossTeam2
    self._boss2 = boss2
end
