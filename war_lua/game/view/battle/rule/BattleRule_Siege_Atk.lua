--[[
    Filename:    BattleRule_Siege_Atk.lua
...
function BattleScene:initBattleUIEx()
    ...
    self._exLayer:getUI("bg2.countBg1"):setVisible(false)
    self._countBg2 = self._exLayer:getUI("bg2.countBg2")
    self._exLayer:getUI("bg2.countBg3"):setVisible(false)
    self._exLayer:getUI("bg2.countBg4"):setVisible(false)

    self._countBg2:setPosition(self._countBg2:getPositionX() + 204, self._exLayer:getUI("bg2.countBg1"):getPositionY())
    ...
    if self._battleInfo.subType == BattleUtils.BATTLE_TYPE_Crusade or self._battleInfo.subType == BattleUtils.BATTLE_TYPE_Biography then
        self._BattleView:getUI("uiLayer.topLayer.timeBg"):setVisible(false)
        self._BattleView:getUI("uiLayer.topLayer.timeLabel"):setPositionY(33)
    end
end
...
function BattleScene:countBgAnim()
    ...
    if self._battleInfo.mode == BattleUtils.BATTLE_TYPE_Siege_Atk_WE then
        local offsetX = 200
...
function BattleScene:onBattleEndEx(res)
    local pro = 0
    local hp1, maxhp1, hp2, maxhp2 = logic:getCurHP()
    res["exInfo"] = {siegeBroken = (logic._aliveSiegesCount == -1), pro = math.floor(hp2 / maxhp2 * 100)}
end
...
function BattleLogic:initLogicEx(procBattle)
    ...
    if not self._battleInfo.siegeBroken then
        ...
        if moatattrObject then
            ...
            local level = moatattrObject[2]
            local attacker = self._teams[2][1].soldier[1]
            local offsetX
            if siegeR_logic then
                offsetX = 80
            else
...
function BattleLogic:heroEnter(callback)
    ...
    self._heros[camp1]:setPos(r and BC.MAX_SCENE_WIDTH_PIXEL - (x - dis) or (x - dis), 12 * 40)
...
function BattleLogic:updateEx()
    ...
    if not BATTLE_PROC then
        if self._battleInfo.mode == BattleUtils.BATTLE_TYPE_Siege_Atk_WE then
            -- 统计伤害，与结果无关
            if self.battleTime > self._lastCountDamageTick then
                self._lastCountDamageTick = self.battleTime + 0.25
                local count = 0
                local teams = self._teams[1]
                for i = 1, #teams do
                    count = count + teams[i].damage
                end
