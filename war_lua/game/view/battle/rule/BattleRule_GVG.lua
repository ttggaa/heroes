--[[
    Filename:    BattleRule_GVG.lua
    Author:      huachangmiao@playcrab.com
    Datetime:    2016-10-13 16:35:01
...
function BattleScene:battleBeginAnimEx()
    ...
    local dx1, dy1 = logic:getRightTeamCenterPoint(BC_reverse)
    local dx2, dy2 = logic:getLeftTeamCenterPoint(BC_reverse)
    dy1 = dy1 + Y_OFFSET
    dy2 = dy2 + Y_OFFSET
    ...
    self._animId = ScheduleMgr:regSchedule(1, self, function(self)
        if self._follow then
            ...
            if logic.battleBeginTick and logic.battleTime > 3 then
                x, y = logic:getAllTeamCenterPoint(BC_reverse)
                self:screenToPos(x, nil, 0.05)
            else
                x, y = logic:getLeftTeamCenterPoint(BC_reverse)
                y = y + Y_OFFSET
