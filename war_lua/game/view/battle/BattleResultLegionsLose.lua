--
-- Author: huangguofang
-- Date: 2018-12-19 17:23:29
--
local BattleResultLegionsLose = class("BattleResultLegionsLose", BasePopView)

function BattleResultLegionsLose:ctor(data)
    BattleResultLegionsLose.super.ctor(self)
    ...
    print(self._hpPercent,"============self._proPercent===",self._proPercent)
    self._stageData = self._battleInfo.stageData or {}
end
function BattleResultLegionsLose:getBgName()
    return "battleResult_bg.jpg"
end
function BattleResultLegionsLose:onInit()
    ...
    local itemCount1 = table.nums(tempRewards)
    local itemCount = 0
    for i = 1, itemCount1 do
        if tempRewards[i][4] <= self._proPercent then
...
function BattleResultLegionsLose:onQuit()
    if self._callback then
        self._callback()
        -- UIUtils:reloadLuaFile("battle.BattleResultLegionsLose")
    end
end

function BattleResultLegionsLose:onCount()
    self._viewMgr:showView("battle.BattleCountView",self._battleInfo,true)
end

local delaytick = {360, 380, 380}
function BattleResultLegionsLose:animBegin(isWin)
    audioMgr:stopMusic()
...
function BattleResultLegionsLose:animNext(isWin, mc2)
    local animPos = self:getUI("bg.animPos")
    ...
    self._timeLabel:setPosition(0, -100)
    mc2:getChildren()[1]:getChildren()[1]:addChild(self._timeLabel,10)
    if self._time then
...
function BattleResultLegionsLose:initMark( )
...
function BattleResultLegionsLose:labelAnimTo(label, src, dest, isTime)
...
function BattleResultLegionsLose.dtor()
    BattleResultLegionsLose = nil
    delaytick = nil
end

return BattleResultLegionsLose