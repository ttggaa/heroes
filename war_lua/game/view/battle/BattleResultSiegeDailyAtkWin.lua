--[[
    @FileName   BattleResultSiegeDailyAtkWin.lua
    @Authors    hexinping
    @Date       2017-09-15 
...
local BattleResultSiegeDailyAtkWin = class("BattleResultSiegeDailyAtkWin", BasePopView)

function BattleResultSiegeDailyAtkWin:ctor(data)
    BattleResultSiegeDailyAtkWin.super.ctor(self)
...
function BattleResultSiegeDailyAtkWin:getBgName()
    return "battleResult_bg.jpg"
end
-- local diffList = {"容易","一般","困难","专家","噩梦",}
function BattleResultSiegeDailyAtkWin:onInit()
    ...
    if tempRewards and itemCount > 0 then
        ...
        for i = 1, itemCount do
            ...
            local item = createItemIconById(self,{itemId = itemId, num = tempRewards[i][3] or tempRewards[i]["num"], itemData = tab:Tool(itemId)})
            item:setScale(0.7)
    ...
    if self._result.exInfo then
        -- local subid = self._result.exInfo.subid or 101
        local subid = self._result.exInfo.siegeCardId
        local diff = self._result.exInfo.diff or 1
        local totalDamage1 = self._result.totalDamage1 or 0
        if self._result.exInfo.pro then
            self._proPercent = self._result.exInfo.pro or 0
            self._count1:setString(lang("LONGZHIGUONANDU_" .. diff))
            self._count2:setString(totalDamage1)
            -- self._proPercent = 
        ...
        if self._result.win then
            local historyD1 = self._result.exInfo.isHisMaxDiff
            --本次造成100%伤害，且之前未开启下一等级
            if historyD1 and tonumber(self._proPercent) == 100  then
                self._upLab11:setString("(")
            ...
            if self._result.exInfo.isHisMaxKillCount then
...
function BattleResultSiegeDailyAtkWin:onQuit()
    if self._callback then
        self._callback()
        -- UIUtils:reloadLuaFile("battle.BattleResultSiegeDailyAtkWin")
    end
end

function BattleResultSiegeDailyAtkWin:onCount()
    self._viewMgr:showView("battle.BattleCountView",self._battleInfo,true)
end

local delaytick = {360, 380, 380}
function BattleResultSiegeDailyAtkWin:animBegin(isWin)
    audioMgr:stopMusic()
    if isWin then
...
function BattleResultSiegeDailyAtkWin:animNext(isWin, mc2)
    local animPos = self:getUI("bg.animPos")
    if isWin then
...
function BattleResultSiegeDailyAtkWin:calutePro(reward)
    local count = #reward
    local sigle = 100/(count-1)
    for i,v in ipairs(reward) do
        v[4] = (i-1) * sigle
    end
end

function BattleResultSiegeDailyAtkWin:initMark(subid)
    local cfgT = clone(tab.siegeBasicBattle)
    local dailySiege 
    ...
    -- if dailySiege then
    --     self:calutePro(dailySiege.reward)
    -- end
    local marks = {}
    local proWidth = self._pro:getContentSize().width
    if dailySiege and dailySiege.reward then
        local reward = dailySiege.reward
...
function BattleResultSiegeDailyAtkWin:labelAnimTo(label, src, dest, isTime)
...
function BattleResultSiegeDailyAtkWin.dtor()
    BattleResultSiegeDailyAtkWin = nil
    delaytick = nil
    -- diffList = nil
end

return BattleResultSiegeDailyAtkWin