--[[
    Filename:    BattleResultArenaLose.lua
...
local BattleResultArenaLose = class("BattleResultArenaLose", BasePopView)

function BattleResultArenaLose:ctor(data)
    BattleResultArenaLose.super.ctor(self, data)

    self._result = data.result
    self._callback = data.callback
    self._battleInfo = data.result
end
function BattleResultArenaLose:getBgName()
    return "battleResult_bg.jpg"
end
function BattleResultArenaLose:onInit()
    ...
    if arenaInfo and arenaInfo.award then
        local rank = arenaInfo.rank
        local preRank = arenaInfo.preRank
        local preHRank = arenaInfo.preHRank
        self._goldLabel:setString(arenaInfo.award.val or "")
        self._bg2:setVisible(true)
    else
    end
    if self._battleInfo and self._battleInfo.arenaInfo and self._battleInfo.arenaInfo.rewards and next(self._battleInfo.arenaInfo.rewards) then
...
function BattleResultArenaLose:initLabel(node,isGreen)      
    if isGreen then
...
function BattleResultArenaLose:onQuit()
	if self._callback then
		self._callback()
	end
end
function BattleResultArenaLose:onShow( )
...
function BattleResultArenaLose:onCount()
	self._viewMgr:showView("battle.BattleCountView",self._battleInfo,true)
    
end

function BattleResultArenaLose:animBegin()
    audioMgr:stopMusic()
...
function BattleResultArenaLose:animNext()
...
function BattleResultArenaLose.dtor()
    BattleResultArenaLose = nil
end

return BattleResultArenaLose