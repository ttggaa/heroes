--[[
    @FileName   BattleResultWorldBossWin
    @Authors    zhangtao
    @Date       2017-09-27 19:42:58
    @Email      <zhangtao@playcrad.com>
    @Description   描述
--]]
local BattleResultWorldBossWin = class("BattleResultWorldBossWin", BasePopView)

function BattleResultWorldBossWin:ctor(data)
    BattleResultWorldBossWin.super.ctor(self)
...
function BattleResultWorldBossWin:getBgName()
    return "battleResult_bg.jpg"
end
function BattleResultWorldBossWin:onInit()
    ...
    if tempRewards and itemCount > 0 then
        ...
        for i = 1, itemCount do
            ...
            -- if item.iconColor.numLab then
            --     item.iconColor.numLab:setVisible(false)
            -- end
    ...
    -- local damageValue = self._result.totalRealDamage1
    local hotBuff = self._result.hotHeroBuff or 0
    -- local heroDamage = self._result.heroDamage1 or 0
    local isInclude = self:isIncludeHotHero()
    local realDamage = self._result.damageValue or 0 --damageValue + heroDamage
    if isInclude then
        -- realDamage = math.floor(realDamage*hotBuff)
        self._buff:setVisible(true)
        self._monsterName3:setPositionX(117)
        self._count3:setPositionX(103)
    else
        self._monsterName3:setPositionX(self._monsterName3:getPositionX() + 80)
        self._count3:setPositionX(self._count3:getPositionX() + 80)
    end
    local damageStr = self:damageFormat(realDamage)
    self._count3:setString(damageStr)

    self._buff:setString("(热点英雄+"..tonumber((hotBuff - 1)*100).."%)")
    self._buff:setPositionX(self._count3:getPositionX() + self._count3:getContentSize().width)

    local totalHurtValue = self._result.totalHurtValue or 0
    totalHurtValue = totalHurtValue + realDamage
    self._count1:setString(self:damageFormat(totalHurtValue))
    --我的排名
    local newRank = self._result["afterRank"] or 0
    self._count2:setString(newRank)
    local oldRank = self._result["beforeRank"] or 0    
...
function BattleResultWorldBossWin:isIncludeHotHero()
    local hotHeros = self._result.hotHeros
    local formHeroId = self._result.hero1["id"]
...
function BattleResultWorldBossWin:damageFormat(damageValue)
...
function BattleResultWorldBossWin:onQuit()
    if self._callback then
        self._callback()
        -- UIUtils:reloadLuaFile("battle.BattleResultWorldBossWin")
    end
end

function BattleResultWorldBossWin:onCount()
    self._viewMgr:showView("battle.BattleCountView",self._battleInfo,true)
end

local delaytick = {360, 380, 380}
function BattleResultWorldBossWin:animBegin(isWin)
    audioMgr:stopMusic()
    if isWin then
...
function BattleResultWorldBossWin:animNext(isWin, mc2)
    local animPos = self:getUI("bg.animPos")
    if isWin then
...
function BattleResultWorldBossWin:labelAnimTo(label, src, dest, isTime)
...
function BattleResultWorldBossWin.dtor()
    BattleResultWorldBossWin = nil
    delaytick = nil
    -- diffList = nil
end

return BattleResultWorldBossWin
