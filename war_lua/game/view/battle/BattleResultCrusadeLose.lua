--[[
    Filename:    BattleResultCrusadeLose.lua
    Author:      huachangmiao@playcrab.com
    Datetime:    2015-12-14 17:46:43
    Description: File description
--]]

local BattleResultCrusadeLose = class("BattleResultCrusadeLose", BasePopView)

function BattleResultCrusadeLose:ctor(data)
    BattleResultCrusadeLose.super.ctor(self, data)
...
function BattleResultCrusadeLose:onInit()
...
--新界面 1.20
function BattleResultCrusadeLose:initUIbutton(btnTable)
    -- print("=========================================")
    -- dump(btnTable)
    for i=1,#btnTable do
...
function BattleResultCrusadeLose:initUIbutton(btnTable)
    ...
    for i=1,#btnTable do
        ...
        self:registerClickEvent(btnTable[i], function()
            ...
            if not fightData.lock then
                self:_quit(1, function ()
                    -- print("===================fightData.toView",fightData.toView)
                    ViewManager:getInstance():returnMain(toIndexView[tonumber(fightData.jump)])
                end)
            else
                -- print("=========================系统没开放")
                ViewManager:getInstance():showTip("功能" .. fightData.openLvl .. "级开启")
...
function BattleResultCrusadeLose:initFightData()
    -- body    
    ...
    -- 9  英雄升星-->英雄星级
    -- dump(self._battleInfo.hero1,"hero1======>>>>")
    local heroStars = 0
    if heroData then
        heroStars = heroData.star
    end
    self._fightData[9].haveScore = heroStars
    ...
    local userLv = userData.lvl or userData.level
...
function BattleResultCrusadeLose:_quit(type, callback)
    if self._callback then
        self._callback(type, callback)
    end
end

function BattleResultCrusadeLose:onQuit()
...
function BattleResultCrusadeLose:onCount()
	self._viewMgr:showView("battle.BattleCountView",self._battleInfo,true)
end

function BattleResultCrusadeLose:animBegin()
    audioMgr:stopMusic()
	audioMgr:playSoundForce("SurrenderBattle")
	-- local mc1 = mcMgr:createViewMC("diguang_commonlose", true)
 --    mc1:setPosition(480, 460)
    ...
    for i=1,#self._button do
        ...
        if not lock then
            ...
            if progress then
                ...
                -- local percentNum = fightData.percent
                -- local pNum = 0              
                -- progress:runAction(cc.Sequence:create(cc.DelayTime:create(0.5+0.1*i+0.2),cc.CallFunc:create(function()
                --     self._schedulerTable[i] = ScheduleMgr:regSchedule(0.01,self,function( )
                --         pNum = pNum + 1
                --         if pNum < percentNum then
                --             progress:setPercent(pNum)
                --         else
                --             progress:setPercent(percentNum)
                --             if self._schedulerTable[i] then
                --                 ScheduleMgr:unregSchedule(self._schedulerTable[i])
                --                 self._schedulerTable[i] = nil
                --             end
                --         end
                --     end)
                --     -- table.insert(self._schedulerTable,scheduler)
                -- end)))    
...
function BattleResultCrusadeLose.dtor()
    BattleResultCrusadeLose = nil
    toIndexView = nil
end

return BattleResultCrusadeLose