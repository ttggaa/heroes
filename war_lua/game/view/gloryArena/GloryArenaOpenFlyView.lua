...
-- 展示方法
-- self._viewMgr:showDialog("league.GloryArenaOpenFlyView", {target = self:getUI("bg.midBg3.pve")}, true) 
local GloryArenaOpenFlyView = class("GloryArenaOpenFlyView",BasePopView)
function GloryArenaOpenFlyView:ctor(param)
...
-- 初始化UI后会调用, 有需要请覆盖
function GloryArenaOpenFlyView:onInit()
end

-- 第一次进入调用, 有需要请覆盖
function GloryArenaOpenFlyView:onShow()

end

function GloryArenaOpenFlyView:getMaskOpacity()
    return 0
end

-- 接收自定义消息
function GloryArenaOpenFlyView:reflashUI(data)
	self._bg = self:getUI("bg")
	self._target = data.target or self._bg
    local mcStar = mcMgr:createViewMC( "rongyaojingjichangkaiqi_kaiqianim", false, false, function (_, sender)
    ...
    --     self:close()
    --     UIUtils:reloadLuaFile("league.GloryArenaOpenFlyView")
    -- end,RGBA8888)
    -- mcStar:setPosition(480,380)
    -- self._bg:addChild(mcStar,99)
end

function GloryArenaOpenFlyView:showFlyAnim( )
    ...
    icon:runAction(cc.Sequence:create(
        ...
        cc.CallFunc:create(function ()
            ...
            self:close()
			UIUtils:reloadLuaFile("gloryArena.GloryArenaOpenFlyView")
        end), 
        cc.RemoveSelf:create(true)))
end

return GloryArenaOpenFlyView